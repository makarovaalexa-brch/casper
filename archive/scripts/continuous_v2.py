"""
PAPER B — G2/G3: generative continuous actor with ON-MANIFOLD action (PLAS) to fix PART U's model exploitation.
MANIFOLD=raw  -> actor emits raw a in R^D (PART U, exploits).
MANIFOLD=plas -> actor emits z in a latent; a=Dec(z), Dec trained on REAL entity embeddings => a stays ON-MANIFOLD.
CRITIC=1 -> add terminal value bootstrap (SHAC). Pathwise gradients through the differentiable user model + frozen encoder.
Diagnostics: decode each emitted a to nearest real concept (distance) [exploitation detector] + train-vs-heldout gap.
Report FULL + TAIL NDCG@10 + Recall@50 vs EIG/random/oracle.
"""
import os, numpy as np, torch, torch.nn as nn
base='C:/dev/phd/casper/data/movielens'; ml=f'{base}/ml-1m'; D=64; DZ=24; T=int(os.environ.get('T',6)); EP=int(os.environ.get('EP',12))
MANIFOLD=os.environ.get('MANIFOLD','plas'); CRITIC=int(os.environ.get('CRITIC',0)); rng=np.random.default_rng(0); torch.manual_seed(0)
U,I,R=[],[],[]
...
import time
# concept centroids (anchors for manifold + decode diagnostic)
...
ctags=[t for t,its in tagitems.items() if len(its)>=30]; Ac=np.stack([Q[np.array(tagitems[t])].mean(0) for t in ctags]).astype(np.float32); cname=[tagn[t] for t in ctags]
Acn=Ac/np.clip(np.linalg.norm(Ac,axis=1,keepdims=True),1e-9,None)
anchors=np.concatenate([Q[order_pop[:1500]],Ac],0).astype(np.float32)         # real-embedding manifold for PLAS
print(f"  {len(ctags)} concepts, {len(anchors)} anchors ({time.time()-t0:.0f}s)",flush=True)
...
# PLAS decoder: train AE on anchors, freeze decoder
class Dec(nn.Module):
    def __init__(s): super().__init__(); s.f=nn.Sequential(nn.Linear(DZ,128),nn.ReLU(),nn.Linear(128,D))
    def forward(s,z): return s.f(z)
dec=Dec()
if MANIFOLD=='plas':
    aenc=nn.Sequential(nn.Linear(D,128),nn.ReLU(),nn.Linear(128,DZ)); oae=torch.optim.Adam(list(aenc.parameters())+list(dec.parameters()),1e-3)
    At=torch.tensor(anchors)
    for ep in range(300):
        idx=torch.randint(0,len(At),(512,)); z=aenc(At[idx]); rec=dec(z); l=((rec-At[idx])**2).mean(); oae.zero_grad(); l.backward(); oae.step()
    for p in dec.parameters(): p.requires_grad_(False)
    print(f"  PLAS decoder trained (AE recon={l.item():.4f})",flush=True)
class Actor(nn.Module):
    def __init__(s,outd): super().__init__(); s.f=nn.Sequential(nn.Linear(D+2,256),nn.ReLU(),nn.Linear(256,256),nn.ReLU(),nn.Linear(256,outd))
    def forward(s,u,t): return s.f(torch.cat([u,t],1))
actor=Actor(DZ if MANIFOLD=='plas' else D); crit=nn.Sequential(nn.Linear(D,128),nn.ReLU(),nn.Linear(128,1))
params=list(actor.parameters())+(list(crit.parameters()) if CRITIC else []); opt=torch.optim.Adam(params,3e-4)
def emit(u,tt):
    z=actor(u,tt)
    return dec(z) if MANIFOLD=='plas' else z
def prep(users):
    ...
    for b,x in enumerate(users):
        ...
        seen=set(ask); nneg=ni-len(seen)-len(tg); m=torch.ones(ni); m[list(seen)]=0
        wtb=wt[b]; wtb[(tgt[b]==0)&(m>0)]=float(posw)/max(nneg,1); wt[b]=wtb
    return Fa,Ya,Ma,tgt,wt
CONS=float(os.environ.get('CONS',0.0)); Act=torch.tensor(Acn)                  # conservative manifold penalty weight
def rollout(Fa,Ya,Ma,Tn,explore=0.1):
    B=Fa.shape[0]; toks=torch.zeros(B,Tn,D+1); tmask=torch.zeros(B,Tn); u=torch.zeros(B,D); Fan=Fa/(Fa.norm(dim=2,keepdim=True)+1e-9); pen=0.
    for t in range(Tn):
        tt=torch.cat([torch.full((B,1),t/8.),torch.full((B,1),float(t))],1); a=emit(u,tt)
        ...
        pen=pen+(1.0-(an@Act.t()).max(1).values).mean()                       # push a onto the concept manifold (max cos -> 1)
        toks=toks.clone(); toks[:,t,:D]=a; toks[:,t,D]=ans; tmask=tmask.clone(); tmask[:,t]=1; u=enc(toks,tmask)
    return u, pen/Tn
def recon_loss(u,tgt,wt): sc=u@Qlt.t()+popbt; bce=nn.functional.binary_cross_entropy_with_logits(sc,tgt,reduction='none'); return (wt*bce).sum()/wt.sum()
trbig=[x for x in trU if len(likes_by_u[x])>=6 and len(rat_by_u[x])>=14]
print(f"train actor MANIFOLD={MANIFOLD} CRITIC={CRITIC} T={T} ({len(trbig)} users)...",flush=True)
for ep in range(EP):
    rng.shuffle(trbig); tot=0.;nb=0
    for b0 in range(0,len(trbig),128):
        Fa,Ya,Ma,tgt,wt=prep(trbig[b0:b0+128]); u,pen=rollout(Fa,Ya,Ma,T); loss=recon_loss(u,tgt,wt)+CONS*pen
        opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(params,5.0); opt.step(); tot+=loss.item();nb+=1
...
def actor_a(u,t):
    with torch.no_grad(): a=emit(torch.tensor(u[None],dtype=torch.float32),torch.tensor([[t/8.,float(t)]])).numpy()[0]
    return a
def answer_of(a,fac,res): an=a/(np.linalg.norm(a)+1e-9); Fn=fac/(np.linalg.norm(fac,axis=1,keepdims=True)+1e-9); s=np.maximum(Fn@an,0); return float((s*res).sum()/(s.sum()+1e-6))
decdist=[]
def run(mode,tail):
    ...
    for x in TE:
        ...
        fac=np.array([Q[j] for j in prof]); res=np.array([rd[j]-mu-bi[j] for j in prof]); toks=[]; asked=[]
        for q in [0,1,2,4,8]:
            while len(toks)<q:
                if mode=='actor':
                    a=actor_a(enc_u_np(toks),len(toks)); ans=answer_of(a,fac,res); toks.append((a,ans))
                    if not tail and q==8 and len(decdist)<2000:
                        an=a/(np.linalg.norm(a)+1e-9); decdist.append(float((Acn@an).max()))   # cos to nearest concept (1=on-manifold)
                else:
...
for tail in [False,True]:
    print(f"\n=== {'TAIL' if tail else 'FULL'}: NDCG@10 / Recall@50 (MANIFOLD={MANIFOLD} CRITIC={CRITIC}) ===",flush=True)
...
print(f"\nEXPLOITATION DETECTOR: actor-direction cos-to-nearest-concept mean={np.mean(decdist):.3f} (1.0=on-manifold; low=off-manifold/exploiting)",flush=True)
