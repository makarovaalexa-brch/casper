...
MIN_EXPECT = 3.0                 # pre-registered solid-support bar (expected member count); the train
                                 # quantile it corresponds to is reported alongside
...
def main():
    ...
    # ---- PRE-REGISTRATION (train-only; written to disk BEFORE scoring) ----
    th = train_thresholds(ctx, Mm, grate, smoke=args.smoke)
    outdir = ctx.outdir if args.smoke else OUTDIR
    os.makedirs(outdir, exist_ok=True)
    path = os.path.join(outdir, "signed_sel_gate.json")
    prereg = {"gate": "signed_sel_triple_gate", "pre_registered": th,
              "decision_rules": {
                  "supported_iff": ["arm1 full@q >= arm1@q2 - eps for all q AND q16 > q2",
                                    "arm2 gain over clipup at q16 < 0.70 * arm1 gain",
                                    "arm3 q16 >= arm3 q2 - eps while clipup declines"],
                  "eps": EPS, "arm2_fraction": 0.70},
              "status": "PRE-REGISTERED, scoring not yet run"}
    json.dump(prereg, open(path, "w"), indent=2)
    log(f"[prereg] thresholds + decision rules written BEFORE scoring -> {path}")
    log(f"[prereg] {th}")
    # ---- ARM 2 counterfeit counts (stratified permutation; volume x popularity preserved) ----
    ...
    counts_cf = np.zeros_like(counts)
    Mm_dense = np.asarray(Mm.todense(), np.float32)
    rng = np.random.default_rng(SEED)
    for r in rows:
        its = ctx.va_tr[r].indices
        x = np.zeros(ctx.ni, np.float32)
        for s in range(N_POP_STRATA):
            k = int((strata[its] == s).sum())
            if k:
                pool = stratum_items[s]
                x[rng.choice(pool, size=min(k, len(pool)), replace=False)] = 1.0
        counts_cf[r] = x @ Mm_dense
    # ---- the four arms (identical interviews; only values/bins differ) ----
    arms_def = {
        "clipup": build_arm_values(ctx, counts, E, clip_v, th, "clipup"),
        "arm1_signed": build_arm_values(ctx, counts, E, clip_v, th, "signed"),
        "arm2_pop_counterfeit": build_arm_values(ctx, counts_cf, E, clip_v, th, "signed"),
    ...
    rule1 = bool(all(a1[q] >= a1[2] - EPS for q in BUDGETS) and a1[16] > a1[2])
    g1 = a1[16] - cu[16]; g2 = a2[16] - cu[16]
    rule2 = bool(g2 < 0.70 * g1) if g1 > 0 else False
    rule3 = bool(a3[16] >= a3[2] - EPS and cu[16] < cu[2] - EPS)
    verdict = "SUPPORTED" if (rule1 and rule2 and rule3) else \
              ("POPULARITY-ARTIFACT (arm2 reproduces the gain)" if rule1 and not rule2 else
               ("CLIP-FIX-ONLY (neutral suffices, signed adds nothing)" if rule3 and not rule1 else
                "KILL (signed does not hold its level)"))
    # paired CI on the decisive contrast (arm1 - clipup at q16)
    d16 = bootstrap_ci(arm_out["arm1_signed"]["_fullvecs"][16] - arm_out["clipup"]["_fullvecs"][16])
    for name in arm_out:
        del arm_out[name]["_fullvecs"]
    res["arms"] = arm_out
    res["decision"] = {"rule1_arm1_holds_and_trends": rule1,
                       "rule2_arm2_lt_70pct": rule2, "arm1_gain_q16": g1, "arm2_gain_q16": g2,
                       "rule3_decline_is_clip_specific": rule3,
    ...
    log(f"[out] -> {path}")
    print(f"\n=== SIGNED-SEL TRIPLE GATE (intercept {res['intercept']['full']:.4f}) ===")
    for name in ("clipup", "arm1_signed", "arm2_pop_counterfeit", "arm3_meh_only"):
        c = arm_out[name]
        print(f"{name:>22}: " + " ".join(f"q{q}={c[str(q)]['full@10']:.4f}" for q in BUDGETS))
    print(f"rules: hold&trend={rule1} | arm2<70%={rule2} (g1={g1:+.4f} g2={g2:+.4f}) | "
          f"clip-specific={rule3}")
    print(f"arm1-clipup@q16 {d16[0]:+.4f} CI {d16[1]} | leak R2 {r2}")
