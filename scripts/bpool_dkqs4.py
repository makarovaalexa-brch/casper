...
def unroll(act,idxs,train=False,collect=False):
    ...
    lam=[1.0 for t in range(T)]; Z=sum(lam); mus=[]; routes=[]        # FIX: EQUAL per-turn weighting (t^2 starved early turns where adaptivity pays)
    for t in range(T):
        ...
        am=w.argmax(1); wh=torch.zeros_like(w); wh[torch.arange(B),am]=1.0
        wst=wh+(w-w.detach())                                          # FIX: STRAIGHT-THROUGH top-1 (forward=clean single concept, backward=softmax grad)
        d_=wst@DcU_t; d_=d_/d_.norm(dim=1,keepdim=True).clamp_min(1e-8)
        Sd=d_@Sig0_t-torch.einsum('bt,btd->bd',torch.einsum('btd,bd->bt',Vst,d_),Vst)
        y=(wst*Yb).sum(1); s2=(wst*S2b).sum(1); denom=(d_*Sd).sum(1)+s2   # clean answer & noise (no w^2 blend subsidy)
        mu=mu+(Sd/denom.unsqueeze(1))*(y-(d_*mu).sum(1)).unsqueeze(1)
        Vst=Vst.clone(); Vst[:,t,:]=Sd/denom.clamp_min(1e-9).sqrt().unsqueeze(1)
        asked=asked.clone(); asked[torch.arange(B),am]=True
...
EPOCHS=70; BS=128   # FIX: real training length (~70*10=700 steps vs the failed 80) + smaller batch
for ep in range(EPOCHS):
    act.train(); ordr=np.random.default_rng(ep).permutation(len(TRi))
    for b in range(0,len(ordr),BS):
        ib=TRi[ordr[b:b+BS]]; loss=unroll(act,ib,train=True); opt.zero_grad(); loss.backward(); opt.step()
...
act.load_state_dict(best[1]); torch.save(best[1],OUT+"/dkqs4_ckpt.pt"); log(f"best ep{best[2]} tail@10 {best[0]:.4f} (ckpt saved)")
