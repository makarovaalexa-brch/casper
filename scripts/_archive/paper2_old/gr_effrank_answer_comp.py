"""
Goodreads COMPOSITE: effective rank (participation ratio) of shelf-concept directions vs item
directions + answerability (concept vs item). Reads base_comp.npz + concepts_comp.npz + Q_svd_comp.npy.
"""
...
print("\n=== ANSWERABILITY (FULL realistic composite cohort) ===")
print(f"  ITEM answer-rate: mean {item_ans.mean():.4f} median {np.median(item_ans):.4f}",flush=True)
print(f"    top-600 popular pool: mean {item_ans[PITEMS].mean():.4f} min {item_ans[PITEMS].min():.4f}",flush=True)
print(f"  CONCEPT answer-rate (>=2 tagged items): mean {conc_ans.mean():.3f} median {np.median(conc_ans):.3f} max {conc_ans.max():.3f}",flush=True)
print(f"    concepts >=0.5: {(conc_ans>=0.5).sum()}/{nc}; >=0.8: {(conc_ans>=0.8).sum()}/{nc}",flush=True)
for ki in np.argsort(-conc_ans)[:12]: print(f"    {conc_ans[ki]:.3f}  {ctags[ki]}")
print(f"\nSUMMARY: concepts effrank={prc_c[0]:.2f}/{D} items effrank={prc_i[0]:.2f}/{D} "
      f"concept-ans mean={conc_ans.mean():.3f} item-ans(pool) mean={item_ans[PITEMS].mean():.4f}",flush=True)
