"""
Goodreads MTC: effective rank (participation ratio) of shelf-concept directions vs item directions,
+ answerability (concept vs item answer-rate) on the FULL realistic cohort. Mirrors ml25m_concepts_full.
Reads base.npz + concepts.npz (Ac, ctags, citems).
"""
import os, time, numpy as np
GR='C:/dev/phd/casper/.cache/goodreads'; t0=time.time()
B=np.load(f'{GR}/base.npz'); uu=B['uu']; ii=B['ii']; cnt=B['cnt']; ni=int(B['ni']); nu=int(B['nu']); trU=B['trU']
Q=np.load(f'{GR}/Q_svd.npy'); D=Q.shape[1]
C=np.load(f'{GR}/concepts.npz',allow_pickle=True); Ac=C['Ac']; ctags=C['ctags']; flat=C['citems_flat']; off=C['citems_off']
...
for ki in topc: print(f"    {conc_ans[ki]:.3f}  {ctags[ki]}")
