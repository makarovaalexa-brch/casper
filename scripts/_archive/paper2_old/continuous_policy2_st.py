...
class EncST(nn.Module):                                                          # set-transformer recommender (MHSA + mean-pool, order-invariant) for LOADREC
...
if os.environ.get('LOADREC') and os.path.exists(os.environ['LOADREC']):          # swap recommender (enc+Ql) -> retrain/eval CASPER-R on a different instrument (e.g. set-transformer V2-ST)
    _r=torch.load(os.environ['LOADREC'])
    if 'st' in _r: enc=EncST(); enc.load_state_dict(_r['st']); enc.eval(); Ql=_r['Ql'].numpy().astype(np.float32); Qlt=torch.tensor(Ql)
    print(f"LOADREC: using set-transformer recommender {os.path.basename(os.environ['LOADREC'])}",flush=True)
...
for ep in range(EP):
    ...
    for b0 in range(0,min(len(trbig),int(os.environ.get('TRN',len(trbig)))),96):   # TRN caps users/epoch for fast epochs on the heavy set-transformer
