...
for r,uid in enumerate(uidsV):
    ...
    rows.append((hlt.astype(np.int64),cc,sel,val,cvalV[r],aki,akr,hlall)); vseq.append((its.astype(np.int64),sv2lv(rat)))
    if len(rows)>=2500: break
UV=beliefs(vseq); US=[(rows[i][0],rows[i][1],rows[i][2],rows[i][3],rows[i][4],UV[i],rows[i][5],rows[i][6],rows[i][7]) for i in range(len(rows))]
log(f"val users {len(US)}")
def ndt(sc,hl): return ndcg10(sc.copy(),list(hl),set(),head,True)
pt=np.mean([ndt(popb,hl) for hl,cc,sel,val,llm,us,aki,akr,hlall in US]); ceil=np.mean([ndt(popb+Wd@us,hl) for hl,cc,sel,val,llm,us,aki,akr,hlall in US]); log(f"popb {pt:.4f} ceiling {ceil:.4f}")
...
poprank=np.argsort(np.argsort(-cnt))   # 0 = most popular
def ndft(sc,hl,hlall):
    return ndcg10(sc.copy(),list(hlall),set(),head,False), ndcg10(sc.copy(),list(hl),set(),head,True)
def run(bank,objective='var'):
    SF=[[] for _ in range(MAXQ+1)]; ST=[[] for _ in range(MAXQ+1)]; picks={'item':0,'concept':0}; iprank=[[] for _ in range(MAXQ)]
    for hl,cc,sel,val,llm,us,aki,akr,hlall in US:
        mu=np.zeros(D); Sig=Sig0.copy(); f,t=ndft(popb+Wd@mu,hl,hlall); SF[0].append(f); ST[0].append(t)
        Q=[]
        if bank in ('concept','both'):
            for c in cc:
                tt=Bc[c,0]*sel[c]+Bc[c,1]*val[c]+Bc[c,2]; k=int(np.searchsorted(EDG[c],tt)); Q.append(('concept',-1,Dc[c],Yc[c,k],max(S2[c,k],1e-3)))
        if bank in ('item','both'):
            for it,rt in zip(aki,akr):
                w=Wcw[it]; nrm=np.linalg.norm(w); d=w/nrm if nrm>0 else w; Q.append(('item',int(it),d,(rt-gm)/max(nrm,1e-6),SIG2I))
        for j in range(min(MAXQ,len(Q))):
            Dcand=np.array([q[2] for q in Q]); SD=Sig@Dcand.T
            vq=np.einsum('dq,dq->q',Dcand.T,SD)+np.array([q[4] for q in Q])
            if objective=='var': gain=np.einsum('dq,dq->q',SD,SD)
            else:
                sc=popb+Wd@mu; sc2=sc.copy(); sc2[head]=-1e30; top=np.argpartition(-sc2,500)[:500]
                lam=1.0/np.log2(2+np.arange(len(top))); A=Wd[top]@SD
                gain=(lam@(A*A))/vq
            pk=int(np.argmax(gain)); kind,iid,dvec,y,s2=Q.pop(pk)
            Sd=Sig@dvec; kk=Sd/(dvec@Sd+s2); mu=mu+kk*(y-dvec@mu); Sig=Sig-np.outer(kk,Sd); picks[kind]+=1
            if kind=='item': iprank[j].append(poprank[iid])
            f,t=ndft(popb+Wd@mu,hl,hlall); SF[j+1].append(f); ST[j+1].append(t)
    ipm=[int(np.median(x)) if x else -1 for x in iprank]
    return [np.mean([z for z in s if z is not None]) for s in SF],[np.mean([z for z in s if z is not None]) for s in ST],picks,ipm

# prior mean 0 (cold=popb). Use empirical Sig0.
pf0=np.mean([ndft(popb,hl,hlall)[0] for hl,cc,sel,val,llm,us,aki,akr,hlall in US]); pt0=np.mean([ndft(popb,hl,hlall)[1] for hl,cc,sel,val,llm,us,aki,akr,hlall in US])
log(f"PROPER popb FULL {pf0:.4f} TAIL {pt0:.4f}")
for bank,obj in [('concept','var'),('both','var'),('concept','voi'),('both','voi'),('item','voi')]:
    cf,ct,picks,ipm=run(bank,obj)
    log(f"[{bank:>7} {obj:>3}] TAIL "+" ".join(f"{i}:{ct[i]:.4f}" for i in range(MAXQ+1))+f" d@q8:{ct[MAXQ]-pt0:+.4f}")
    log(f"[{bank:>7} {obj:>3}] FULL "+" ".join(f"{i}:{cf[i]:.4f}" for i in range(MAXQ+1))+f" d@q8:{cf[MAXQ]-pf0:+.4f} picks {picks} item-poprank/turn {ipm}")
