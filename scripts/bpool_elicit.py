...
for r,uid in enumerate(uidsV):
    ...
    sel,val,e=selval(ki.astype(np.int64),kr-item_mean[ki])          # KNOWN HALF only (no target leak)
    cc=np.where(ansV[r]&usable&(e>=TAU))[0]                          # answerable + exposed (non-refused)
    if len(cc)==0: continue
    rows.append((hlt.astype(np.int64),cc,sel,val,cvalV[r])); vseq.append((its.astype(np.int64),sv2lv(rat)))
    if len(rows)>=2500: break
UV=beliefs(vseq); US=[(rows[i][0],rows[i][1],rows[i][2],rows[i][3],rows[i][4],UV[i]) for i in range(len(rows))]
log(f"val users {len(US)}")
def ndft(sc,hl): return ndcg10(sc.copy(),list(hl),set(),head,False), ndcg10(sc.copy(),list(hl),set(),head,True)
def mn(a): a=[x for x in a if x is not None]; return np.mean(a) if a else float('nan')
pf0=mn([ndft(popb,hl)[0] for hl,cc,sel,val,llm,us in US]); pt0=mn([ndft(popb,hl)[1] for hl,cc,sel,val,llm,us in US])
cf=mn([ndft(popb+Wd@us,hl)[0] for hl,cc,sel,val,llm,us in US]); ct=mn([ndft(popb+Wd@us,hl)[1] for hl,cc,sel,val,llm,us in US])
log(f"popb FULL {pf0:.4f} TAIL {pt0:.4f} | ceiling FULL {cf:.4f} TAIL {ct:.4f}")
...
GMAP=None
MAXQ=8
def obs(mode,c,sel,val,llm,us):
    if mode=='selval': t=Bc[c,0]*sel[c]+Bc[c,1]*val[c]+Bc[c,2]; k=int(np.searchsorted(EDG[c],t)); return Yc[c,k],max(S2[c,k],1e-3)
    if mode=='sel':    t=Bc[c,0]*sel[c]+Bc[c,2]; k=int(np.searchsorted(EDG[c],t)); return Yc[c,k],max(S2[c,k],1e-3)
    if mode=='oracle': return us@Dc[c], 0.05
    if mode=='meh':    return Bc[c,2], 1.0     # SEL/VAL=0 -> intercept-level -> control
def run(mode,order='greedy'):
    SF=[[] for _ in range(MAXQ+1)]; ST=[[] for _ in range(MAXQ+1)]
    for hl,cc,sel,val,llm,us in US:
        mu=np.zeros(D); Sig=Sig0.copy(); f,t=ndft(popb+Wd@mu,hl); SF[0].append(f); ST[0].append(t); pool=list(cc)
        for j in range(min(MAXQ,len(cc))):
            if order=='greedy': Dcand=Dc[pool]; g=np.einsum('kd,kd->k',Dcand@Sig,Dcand); c=pool[int(np.argmax(g))]
            else: c=pool[int(np.random.default_rng(j*7+1).integers(len(pool)))]
            pool.remove(c); dvec=Dc[c]; y,s2=obs(mode,c,sel,val,llm,us); Sd=Sig@dvec; k=Sd/(dvec@Sd+s2); mu=mu+k*(y-dvec@mu); Sig=Sig-np.outer(k,Sd)
            f,t=ndft(popb+Wd@mu,hl); SF[j+1].append(f); ST[j+1].append(t)
    return [mn(s) for s in SF],[mn(s) for s in ST]
Sig0=np.cov(UT.T)+1e-3*np.eye(D); mu0used=False
# prior mean 0 (cold=popb). Use empirical Sig0.
for mode,order in [('selval','greedy'),('selval','rand'),('oracle','greedy'),('meh','greedy')]:
    cf,ct=run(mode,order)
    log(f"[{mode:>7} {order:>6}] TAIL "+" ".join(f"{i}:{ct[i]:.4f}" for i in range(MAXQ+1))+f" d@q8:{ct[MAXQ]-pt0:+.4f}")
    log(f"[{mode:>7} {order:>6}] FULL "+" ".join(f"{i}:{cf[i]:.4f}" for i in range(MAXQ+1))+f" d@q8:{cf[MAXQ]-pf0:+.4f}")
