"""
Goodreads MTC: V1-style attention fold-in set-encoder (mirrors ml25m_train_enc.py / encoder_recon).
d=64 attention pooling over revealed (Q[item], residual) tokens -> user vector u; FROZEN decoder
popb + Q.u; IPS/tail-weighted BCE reconstruction of unrevealed like-set; masked random-count reveals.
RESUMABLE (chunked). Best-val on 500-user val cohort, encoder full-profile fold, NDCG@KVAL full catalogue.
Durable: enc_v1_gr.pt (best), enc_v1_gr_state.pt (resume), enc_v1_gr_peak.txt.
Env: EP_CHUNK(2) EP_MAX(12) K(12) BATCH(384) KVAL(142) NEVAL_VAL(500).
"""
...
K=int(os.environ.get('K',12)); BATCH=int(os.environ.get('BATCH',384))
EP_CHUNK=int(os.environ.get('EP_CHUNK',2)); EP_MAX=int(os.environ.get('EP_MAX',12))
KVAL=int(os.environ.get('KVAL',142)); NEVAL_VAL=int(os.environ.get('NEVAL_VAL',500))
...
Q=np.load(f'{GR}/Q_svd.npy').astype(np.float32); bi=np.load(f'{GR}/bi_svd.npy').astype(np.float32); Qt=torch.tensor(Q)
...
print(f"train users (>=K+1 rated): {len(trbig)} / {len(trU)}",flush=True)
class Enc(nn.Module):
    def __init__(s):
...
st=f'{GR}/enc_v1_gr_state.pt'; bestf=f'{GR}/enc_v1_gr.pt'; peakf=f'{GR}/enc_v1_gr_peak.txt'
ep0=0; best=-1.0
