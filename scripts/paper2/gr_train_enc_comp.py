"""
Goodreads COMPOSITE: V1 attention fold-in set-encoder (same recipe as gr_train_enc.py). Reads
base_comp.npz / Q_svd_comp.npy / bi_svd_comp.npy. Best-val on 500-user val cohort, NDCG@KVAL full cat.
Durable: enc_v1_comp.pt (best), enc_v1_comp_state.pt (resume), enc_v1_comp_peak.txt.
Env: EP_CHUNK(2) EP_MAX(12) K(12) BATCH(384) KVAL(set to composite K) NEVAL_VAL(500).
"""
...
print(f"CSR built ({time.time()-t0:.0f}s); nu={nu} ni={ni} ratings={len(uu_s)} KVAL={KVAL}",flush=True)
...
st=f'{GR}/enc_v1_comp_state.pt'; bestf=f'{GR}/enc_v1_comp.pt'; peakf=f'{GR}/enc_v1_comp_peak.txt'
ep0=0; best=-1.0
