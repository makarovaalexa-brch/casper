...
class Enc(nn.Module):                                      # V2-A: GRU + multi-head self-attention (DAHCR), SAME I/O as V1's attn-pool (architecture-only change)
    def __init__(s,H=128):
        super().__init__(); s.inp=nn.Linear(D+1,H); s.attn=nn.MultiheadAttention(H,4,batch_first=True); s.gru=nn.GRU(H,H,batch_first=True); s.out=nn.Linear(H,D)
    def forward(s,t,m):
...
torch.save({'era':enc.state_dict(),'Ql':Qp.detach()}, f'{base}/.cache/enc_concept_gru.pt'); print("SAVED enc_concept_gru.pt (V2-A: GRU+attn, LOADREC-compatible {era,Ql}; frozen V1 Ec; same data/loss as V1)",flush=True)
