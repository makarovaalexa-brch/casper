"""
Eval the distilled method checkpoint with ARGMAX vs SAMPLING (+ temperatures)
on the hard-LOO metric. Sampling = learned exploration (Thompson analogue) to
avoid argmax-collapse onto the single most-likely-rated item. No retraining.
"""
import os, sys
...
def main():
    ...
    def run(mode, temp=1.0):
        r = np.random.default_rng(1); curves = []
        for (prof, tgt, cand) in cases:
            rev = []; asked = {tgt}; row = [hit([], cand)]
            for _ in range(T):
                with torch.no_grad():
                    lg = actor(torch.from_numpy(state_from(rev)).unsqueeze(0))[0].numpy()
                lg[list(asked)] = -1e9; lg[nt:] = -1e9        # movies only
                if mode == 'argmax':
                    a = int(np.argmax(lg))
                else:
                    p = np.exp((lg - lg.max()) / temp); p[~np.isfinite(p)] = 0; p /= p.sum()
                    a = int(r.choice(ni, p=p))
                asked.add(a); v = prof[a]
                if not np.isnan(v): rev.append((a, float(v)))
                row.append(hit(rev, cand))
            curves.append(row)
        c = np.array(curves).mean(0)
        print(f"  {mode}{'' if mode=='argmax' else f'(T={temp})':>8} turn0={c[0]:.3f} t5={c[5]:.3f} t15={c[-1]:.3f} AUC={c.mean():.4f}", flush=True)

    print(f"=== method eval (argmax vs sampling), {len(cases)} users ===")
    run('argmax'); run('sample', 1.0); run('sample', 0.5); run('sample', 2.0)
    print("  refs: HELF 0.497/0.523 | thompson 0.485/0.537 | popularity 0.488/0.510 | ceiling 0.647")
