...
| ATTR-ORACLE | Attribute-only (genre+concept) oracle: is there ANY headroom? | attr_oracle.py | ✅ KEY | Calibrated instrument, held-out, 120u, T=8. realizable attributes (affinity order) 0.301→0.304 (+0.003 dead); **ATTRIBUTE ORACLE (18 genres + 100 genome concepts, privileged greedy) 0.301→0.332 (+0.031 = LOW CEILING)**; ITEM ORACLE 0.301→0.485 (+0.184) for reference. VERDICT: attribute elicitation has a LOW ceiling (~6× below items) even with a perfect selector → NO attribute-elicitation baseline (EAR/SCPR/UNICORN/PEBOL) can beat +0.031 → SKIP building them; the oracle already bounds them. CAUSE (proven at oracle level, not answer-model/selector): a genre/concept centroid is a COARSE direction (avg of 100s of items) — localizes a broad region, can't pinpoint specific held-out items; items are precise points (~6× better). Paper A: elicitation story = ITEMS; attributes = clean rigorous NEGATIVE (oracle is the proof). |
