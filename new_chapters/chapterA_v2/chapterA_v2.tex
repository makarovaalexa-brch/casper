...
Mult-VAE~\cite{liang2018variational}         & \cmark & \cmark & \xmark & \pmark & \xmark & \multicolumn{2}{c}{---} & queued & @100 $0.426$ (ours $0.422$, near-snap $-.004$) \\
...
RBMF / Functional-MF~\cite{liu2011wisdom,zhou2011functional} & \xmark & \cmark & \xmark & \xmark & \xmark & \multicolumn{2}{c}{---} & queued (best-effort) & n/r (elicitation RMSE) \\
...
\emph{blank} or \emph{measurably weak}. Its one runnable core, the Golbandi node
recommender, lands at $0.3065/0.1895$, and carries an \xmark\ on R1. The rest of the
...
trained operator rather than the single unified interface R3 asks for; the belief layer is fitted and gated but its covariance is not yet
exploited in the ranking path; and per-question monotonicity is observed but not yet certified on this checkpoint. Three half-marks, each
...
\begin{figure}[t]
  ...
  \caption{\textbf{Graded input helps over binarized input, most on the short interview} (ML-25M interview
    harness, full NDCG@10 vs.\ interview length $k$). The same
    ...
    corrupted), not value from gradation. Mirroring the star signs (flip) costs up to $-0.29$. }
