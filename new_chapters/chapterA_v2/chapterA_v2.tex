...
Because the map is defined on any string, it also accepts phrases that are not concepts in our vocabulary
at all. Asking for ``courtroom drama'' returns \emph{Indictment: The McMartin Trial}, \emph{Primal Fear},
\emph{\ldots And Justice for All} and \emph{The Verdict}; ``coming of age in the suburbs'' returns
\emph{The Way, Way Back}, \emph{The Edge of Seventeen} and \emph{Valley Girl}; ``movies about grief''
returns \emph{Rabbit Hole}, \emph{Mean Creek} and \emph{The Woodsman}; ``spy during the cold war'' returns
\emph{Funeral in Berlin} and \emph{Where Eagles Dare}. \TODO{re-select the probes and re-report: several
of the phrases above are genome tags verbatim (``courtroom drama'' is one), and the probe adapter was
fitted on all tags, so as run these demonstrate recall of a fitted direction rather than open-vocabulary
generalisation --- pending the nearest-tag and leave-the-neighbour-out measurement}
