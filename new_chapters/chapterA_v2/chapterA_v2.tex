...
To make that meaningful rather than decorative we report, for each phrase, the \emph{closest} concept we
actually hold and how close it is---so the reader can judge whether the phrase is doing new work or merely
paraphrasing something already curated. The four phrases below all sit well away from any concept in the
vocabulary (nearest-neighbour cosine $\le0.7$; every phrase within $0.75$ of a genome tag is excluded from
this table, which removes ``courtroom drama'' and ``dinosaurs'' as tags in their own right):

\begin{center}\small
\setlength{\tabcolsep}{6pt}
\begin{tabular}{p{3.5cm}p{2.6cm}c p{6.0cm}}
\toprule
...
``slow burn character study'' & \emph{character study} & $0.52$ & Spring; Let Me In; The Wailing; Let the Right One In \\
``spy during the cold war'' & \emph{cold war} & $0.67$ & Where Eagles Dare; Funeral in Berlin; Sink the Bismarck! \\
``coming of age in the suburbs'' & \emph{coming of age} & $0.69$ & The Way, Way Back; Manic; Dogfight; The Edge of Seventeen \\
\bottomrule
\end{tabular}
\end{center}

\noindent The first row is the clearest case. The nearest concept in the entire vocabulary to ``movies
about grief'' is \emph{feel good movie}---tonally its opposite---and the phrase's direction shares
\emph{none} of its top ten items with that concept's curated direction, yet the films it returns are the
right ones. The phrase is not retrieving a stored concept; it is being placed in the latent on its own
terms.
