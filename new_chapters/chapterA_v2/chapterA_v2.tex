...
gap map. Table~\ref{tab:taxonomy} is the compact conceptual overview; the chapter's anchor is the per-system
...
Most-Popular                                & \xmark & \cmark & \xmark & \xmark & \pmark & $.2851$ & $.0589$ & measured   & n/r (non-personalised floor) \\
Item-kNN (cosine)                           & \xmark & \cmark & \xmark & \xmark & \pmark & $.4272$ & $.2208$ & measured   & @100 $\sim0.36$ class~\cite{cremonesi2010performance} \\
PureSVD / iALS-WMF                          & \xmark & \cmark & \xmark & \xmark & \pmark & $.4272$ & $.3172$ & measured (iALS) & @100 $0.386$ (WMF, ML-20M) \\
SLIM                                        & \pmark & \cmark & \xmark & \xmark & \pmark & \multicolumn{2}{c}{---} & not-run & @100 $0.401$ (ML-20M) \\
EASE~\cite{steck2019ease}                    & \cmark & \cmark & \xmark & \xmark & \pmark & $\mathbf{.5078}$ & $\mathbf{.3394}$ & measured & @100 $0.420$ (ours \textbf{PASS} $+.0003$) \\
EDLAE~\cite{steck2020edlae}                  & \cmark & \cmark & \xmark & \xmark & \pmark & $\mathbf{.5230}$ & $\mathbf{.3464}$ & measured & @100 $\approx0.42$--$0.44$ class \\
GF-CF / BSPM / Turbo-CF~\cite{shen2021gfcf,choi2023bspm,park2024turbocf} & \pmark & \cmark & \xmark & \xmark & \pmark & \multicolumn{2}{c}{---} & not-run & n/r (ML-20M not reported) \\
\midrule
...
RecVAE~\cite{ren2020recvae} ($d{=}512$)      & \cmark & \cmark & \xmark & \pmark & \xmark & $0.4998$ & $0.3443$ & measured$^{\ddagger}$ & @100 $0.442$ (ours \textbf{PASS} $+.0005$) \\
SASRec / BERT4Rec~\cite{kang2018sasrec,sun2019bert4rec} & \pmark & \cmark & \xmark & \xmark & \xmark & \multicolumn{2}{c}{---} & not-run & n/r (next-item; repl.~\cite{petrov2022replicability}) \\
TaNP~\cite{lin2021tanp}                       & \pmark & \cmark & \xmark & \cmark & \xmark & \multicolumn{2}{c}{---} & not-run & n/r (own cold-start benchmark) \\
\midrule
\multicolumn{10}{l}{\emph{(c) Elicitation / CRS systems --- the belief corner}}\\
EDDI / Partial-VAE~\cite{ma2019eddi}         & \xmark & \cmark & \pmark & \cmark & \xmark & \multicolumn{2}{c}{---} & cannot-rank-full & n/r (non-recsys; UCI/MNIST) \\
Golbandi tree~\cite{golbandi2011adaptive}    & \xmark & \cmark & \xmark & \xmark & \xmark & \multicolumn{2}{c}{---} & not-run & n/r (Netflix RMSE) \\
RBMF / Functional-MF~\cite{liu2011wisdom,zhou2011functional} & \xmark & \cmark & \xmark & \xmark & \xmark & \multicolumn{2}{c}{---} & not-run & n/r (elicitation RMSE) \\
B\i y\i k soft-attributes~\cite{biyik2023soft} & \xmark & \cmark & \pmark & \cmark & \pmark & \multicolumn{2}{c}{---} & not-run & n/r (own elicitation task) \\
ConTS~\cite{li2021seamlessly}                & \xmark & \cmark & \pmark & \cmark & \pmark & \multicolumn{2}{c}{---} & not-run & n/r (cold-start CRS metrics) \\
BCIE~\cite{toroghi2023bcie}                  & \xmark & \cmark & \pmark & \cmark & \pmark & \multicolumn{2}{c}{---} & not-run & n/r (critique metrics) \\
PEBOL~\cite{austin2024bayesian}              & \xmark & \cmark & \cmark & \pmark & \xmark & \multicolumn{2}{c}{---} & needs-LLM & MRR@10 $0.27$ vs $0.17$ (cold-start only) \\
LLM zero-shot CRS~\cite{he2023large}         & \xmark & \cmark & \cmark & \xmark & \xmark & \multicolumn{2}{c}{---} & needs-LLM & n/r (weak collaborative signal) \\
\midrule
\multicolumn{10}{l}{\emph{(d) Ours}}\\
Set-encoder tower (pbC)                      & \pmark & \cmark & \pmark & \xmark & \pmark & $.4946$ & $.3372$ & recorded$^{\dagger}$ & n/a (in-house) \\
RecVAE-class $d{=}512$ ruler                 & \pmark & \cmark & \xmark & \pmark & \xmark & $.4998$ & $.3443$ & measured$^{\ddagger}$ & n/a (in-house; ML-20M snap \textbf{PASS} $0.442$) \\
...
consistent with Table~\ref{tab:taxonomy}). \emph{full/tail NDCG@10} is on \emph{our} ML-25M half-split harness
(seed-averaged $\{1,2,3,7,11\}$; test cohort $=$ held-out users minus the 300 quarantined study users), scored
with the decoder's learned bias. Status: \emph{measured} (from our baseline JSONs); \emph{queued} (\TODO{}, run
pending); \emph{recorded}$^{\dagger}$ (carried from record and NOT currently reproducible from version
control---see the verification caveat in \S\ref{sec:expdesign}; the reproducing in-house anchor is pb2 at
$0.4917$ full); or ``---'' with a one-word reason
(\emph{not-run}, \emph{cannot-rank-full}, \emph{needs-LLM}). The final column is each system's own published
full-profile anchor on its own split/metric---\emph{not} comparable to our ruler until bridged (\S\ref{sec:bridge});
``n/r'' where the literature reports none. RecVAE$^{\ddagger}$: measured on this ruler (our $d{=}512$ training
run, Jul 2026; its saved test record includes the matching Most-Popular cross-check on the same splits). The
checkpoint weights were later purged, so the run is not re-executable until retrained---but the implementation
family passed the ML-20M published-number snap, so the measurement stands as a measurement, not a citation. The
``ours'' block repeats this model as the ruler row. No number is invented: a value appears only if it is in a baseline JSON, in
the landscape file with a citation, or is one of the two recorded in-house anchors.}
\label{tab:master}
\end{sidewaystable}

\paragraph{The one column that is blank for an entire block.}
Read Table~\ref{tab:master} down its measured-accuracy column. Every system that carries a number---a full/tail
NDCG@10 on our shared ML-25M ruler---sits in block (a) or (b): the item-only linear and amortised recommenders.
The entire belief corner, block (c)---EDDI, Golbandi, RBMF/Functional-MF, Bıyık, ConTS, BCIE, PEBOL, the
LLM-zero-shot CRS---has a \emph{blank} accuracy column: not one of these systems has a full-catalogue top-N number
on any shared ruler, and its R1 cell is \xmark\ in every row. This is not an accident of our harness. Most CRS and
elicitation papers never report full-catalogue top-N accuracy at all (they report elicitation RMSE, cold-start
MRR/MAP on a shortlist, or critique metrics), and several \emph{cannot produce it by construction}: a per-item
Beta posterior (PEBOL) or a decision tree over rating queries (Golbandi) does not induce a ranking over the whole
catalogue, and an LLM CRS needs an API call per candidate to score one. Conversely, every system with a competitive
number is item-only and point-estimate---\xmark\ on R3 and R4. The two blocks are disjoint: no row combines a
\cmark\ (or even a filled-in number) in the accuracy column with \cmark s across R3--R5. The top-right cell of the
...
\emph{Verification caveat (updated after a forensic re-check):} the in-house anchors have unequal provenance.
The RecVAE-$d512$ weights are no longer on disk (its $0.4998/0.3443$ stands on its saved test record; the
implementation that would rebuild it passed the ML-20M snap). For the belief-pool encoders the situation is
worse and we state it plainly: re-scoring \texttt{pbC} under every git-recoverable version of its training-time
forward yields $0.2428/0.1505$ (test) --- far below the recorded $0.4946/0.3372$ --- because the forward pass
that produced the recorded number lived only in an uncommitted working tree and was overwritten. The recorded
belief-pool anchors are therefore \emph{unreproducible from version control} and are quoted for history only.
The one in-house anchor that reproduces from committed code is the attention-pool item-only encoder
\texttt{pb2}: measured $0.4917$ full ($0.3071$ tail) against a recorded $0.4852/0.3295$. All instrument claims
in this chapter will be re-based on a tower whose forward pass is committed and whose metric re-derives on demand.
