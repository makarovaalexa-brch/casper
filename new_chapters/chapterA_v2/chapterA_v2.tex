...
ML-25M harness). Our two in-house anchors---a set-encoder tower at full/tail NDCG@10 $0.4917/0.3071$ and a
...
RecVAE~\cite{ren2020recvae} ($d{=}512$)      & \cmark & \cmark & \xmark & \pmark & \xmark & $0.4998$ & $0.3443$ & measured & @100 $0.442$ (ours \textbf{PASS} $+.0005$) \\
...
TaNP~\cite{lin2021tanp} (MeLU / CDRNP line)   & \pmark & \cmark & \xmark & \cmark & \xmark & \multicolumn{2}{c}{---} & not-run & n/r (own cold-start benchmark) \\
\midrule
...
ICF~\cite{zhao2013interactive}               & \xmark & \cmark & \xmark & \cmark & \pmark & \multicolumn{2}{c}{---} & not-run & n/r (online CF; own protocol) \\
...
GATE~\cite{li2023eliciting}                  & \xmark & \cmark & \cmark & \xmark & \xmark & \multicolumn{2}{c}{---} & needs-LLM & n/r (elicitation-only; no full ranking) \\
...
Set-encoder tower (pb2)                      & \pmark & \cmark & \xmark & \xmark & \pmark & $.4917$ & $.3071$ & measured & n/a (in-house) \\
RecVAE-class $d{=}512$ ruler                 & \pmark & \cmark & \xmark & \pmark & \xmark & $.4998$ & $.3443$ & measured & n/a (in-house; ML-20M snap \textbf{PASS} $0.442$) \\
...
with the decoder's learned bias. Status: \emph{measured} (run on this ruler); \emph{queued} (\TODO{}, run
pending); or ``---'' with a one-word reason
...
``n/r'' where the literature reports none. The RecVAE row is our own $d{=}512$ training run on this ruler; the
``ours'' block repeats it as the ruler row. Omitted systems: UNICORN-line graph-RL CRS (no full-profile claim;
covered by the taxonomy discussion, \S\ref{sec:gap}); MeLU/CDRNP (folded into the TaNP row as one meta-learning
line). No number is invented: a value appears only if it was measured on this ruler or carries a citation.}
\label{tab:master}
\end{sidewaystable}
...
\begin{tikzpicture}[>=Latex,scale=1.0]
  ...
  \node[draw,rounded corners,fill=blue!6,font=\scriptsize,align=center] (ease) at (1.6,7.0) {EASE / Mult-VAE\\/ RecVAE / EDLAE};
  ...
  \node[draw,rounded corners,fill=orange!8,font=\scriptsize,align=center] (conts) at (7.6,2.2) {ConTS / BCIE\\/ ICF};
  \node[draw,rounded corners,fill=orange!8,font=\scriptsize,align=center] (pebol) at (9.4,1.4) {PEBOL / GATE\\/ LLM-CRS};
  \node[draw,rounded corners,fill=orange!8,font=\scriptsize,align=center] (golb) at (5.9,1.2) {Golbandi /\\RBMF trees};
  % ours today (measured/recorded towers): high accuracy, partial compatibility
  \node[draw,rounded corners,fill=green!8,font=\scriptsize,align=center] (ours) at (5.3,6.6) {ours today\\(pb2 / $d512$)};
  % target (top-right)
...
our set-encoder tower reaches full/tail NDCG@10 $0.4917/0.3071$ and our RecVAE-class $d{=}512$ ruler reaches
...
encoder ($0.4917/0.3071$) and our RecVAE-class ruler ($0.4998/0.3443$). All rows share the same split, cohort,
tail definition, seed set, and NDCG@10 routine (verified by code inspection). The instrument's R1 obligation is
therefore to close a $\sim3$-point full-profile gap to EDLAE ($0.5230$; EASE $0.5078$), not merely to tie
RecVAE. We report this rather than excluding the stronger baselines.
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt]
  \item \textbf{Metric non-comparability until the bridge is run.} Our $0.4917/0.3071$ and $0.4998/0.3443$ are NDCG@10
