...
remaining per-build gates have all been run on a pre-certification snapshot of the same architecture: most pass,
	extbf{G1 fails} on its calibration clause, and G6/G8 are partial; we report the failure and the partials in the same
detail as the passes. The
...
been run} on a pre-certification snapshot of the same architecture: most pass, \textbf{G1 fails} its calibration
clause, and G6/G8 are partial (Table~\ref{tab:batteryresults}). Results measured on that earlier snapshot are labelled \REDO{} where the
...
($R^2$ of a linear read-out) rises from $0.025$ to $0.249$ once signed answers are folded, so a signed-answer build is
...
\paragraph{A retraction: ``items win the long interview'' was an artifact of a broken fold.} An earlier version of this
section reported the opposite shape past $q{=}4$---that popularity items pulled away from a concept plateau near
$0.147$ from the eighth question onward---from a strategy$\times$channel suite run on a superseded concept fold. That
fold was trained under a recipe whose concept-capture rate was $1$--$6\%$; repairing the recipe (a signed four-band
answer plus a curriculum over the number of concepts and dropped members) raised capture to roughly $25\%$, and on a
re-measurement the long-interview reading did not survive. We record the retraction rather than quietly restating the
numbers, because it is the same lesson the rest of this section is about: a channel's measured ceiling is a property of
the operator it is measured through, and a weak operator manufactures a confident negative result. We also decline to
replace it with a second heuristic pairing measured on the repaired fold, which would repeat the original mistake with
a different set of arbitrary strategies; the best-static criterion described above is the replacement.
...
The other two amortised anchors are reported with the same candour, because a bridge that only ever passes is not a
bridge. \textbf{Mult-VAE} reaches NDCG@100 $0.4223$ against the published $0.426$~\cite{liang2018variational}
...
The battery is proposed in \S\ref{sec:reqs} as a portable certificate, so it owes its own results. Table~\ref{tab:batteryresults}
reports every gate that has been run. All rows except G0 and C1 are measured on the pre-certification
snapshot of the tower architecture and are provisional in the sense of \S\ref{sec:concept}; the re-run on
the certified checkpoint is queued. We report the two partial verdicts and the one miss in the same detail
as the passes, because a battery whose author only publishes its passes certifies nothing.
...
G1 & \emph{all three clauses} (conjunctive, as defined in \S\ref{sec:reqs}) & --- & \textbf{FAIL} \\
...
\;\;G1c & $\rho(\text{belief-}\sigma,\text{NLL})\ge0.25$ & $\rho=0.2393$ & \emph{miss} \\
...
G4$^\S$ & refusal boundedness, same channel & $\alpha_{\text{refuse}}/\alpha_{\text{answer}}=0.0007$ vs a $\le0.05$ bar & \textbf{PASS}$^\S$ \\
G5$^\S$ & concept lifts members, beats its popularity projection & whitened-direction membership AUC $0.94$ (raw $0.44$); cold fold clears the intercept $0.1308>0.1279$ & \textbf{PASS}$^\S$ \\
G6 & wrong-user / shuffle / placebo / duplicate & shuffled $+0.0371$, placebo $+0.0168$ (both CI-clean), duplicate assert fired; \emph{wrong-user $-0.0986$ below intercept, not $\approx$ intercept} & \emph{partial} \\
G8 & $\bSigma$ beats isotropic $c\Imat$; order-invariant & $+0.0089$ at $q{=}16$ (CI $[0.0071,0.0107]$), also wins $q{=}4,8$, \emph{loses $q{=}2$}; order deviation $5\!\times\!10^{-7}$ & \emph{partial} \\
...
\bottomrule
\end{tabular}
\caption{\textbf{The acceptance battery, measured.} \textbf{G1 is scored as a FAIL}: \S\ref{sec:reqs}
defines it as one gate with three conjunctive clauses, and one clause misses, so the gate does not pass---we
show the sub-gates for diagnosis, not to convert a failure into a tally of passes. (For the record, G1c's
$0.25$ bar was set relative to an abandoned learned uncertainty head at $\rho=0.219$, which the analytic
belief beats by $+0.020$ while falling $0.011$ short of the rounded bar; that is context, not a pass.)
$^\ddagger$\textbf{G2's arms ask only questions each user can already answer}---the bank is filtered per
user to their answerable subset, so all $16$ questions land. It therefore measures \emph{ingestion under
oracle answerability} and is silent on question \emph{availability}, which a deployed static interview
must face: the same tower under one fixed sequence for all users gets only $3.6$ of $16$ questions
answered and reaches $0.2098$ rather than $0.2724$. Both numbers are correct; they answer different
questions, and G2 answers the easier one. $^\S$G4 and G5 pass on the clauses that real data can exercise:
G4's confidence-ordering clause is untestable here (no within-channel confidence signal exists in
MovieLens), and G5's member-bag comparison clause is not re-measured on this build. A pass on a subset of
a criterion is weaker than a pass, and we mark it. G6's wrong-user arm falls far \emph{below} the
intercept rather than near it, failing the criterion as written; we record the partial and decline to
argue it into a pass. G8 shows $\bSigma$ is load-bearing from $q{=}4$ onward but not at $q{=}2$.
$^\dagger$C1 passes \emph{with a disqualification} (\S\ref{sec:bridge}). All rows but G0/C1 are on the
pre-certification snapshot.}
\label{tab:batteryresults}
\end{table}

\paragraph{An awkward result we report rather than bury: random question selection is a strong baseline.}
