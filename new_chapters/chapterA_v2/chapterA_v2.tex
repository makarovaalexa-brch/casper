...
\begin{enumerate}[leftmargin=1.5em,itemsep=1.5pt]
  ...
  \item \textbf{The instrument.} A design for a \emph{frozen} certified shallow-SOTA tower with an analytic
        ...
        approximate-posterior near-miss of B\i y\i k et al.~\cite{biyik2023soft}.
...
near-miss is B\i y\i k et al.~\cite{biyik2023soft}: it fails R1 (a co-trained item space, not a frozen certified
...
B\i y\i k~\cite{biyik2023soft} (co-trained) and the guarantee that G0 (full strength preserved) is a property of the
...
Because a signed answer built partly from \emph{absence} can leak how much a user has watched, we pair the
criterion with a volume-invariance requirement, backed by a probe: the recoverability of log watch-volume
from the belief latent ($R^2$ of a linear read-out) must stay low once signed answers are folded. It
currently does not---$R^2$ reaches $0.249$ against a pre-registered bar of $\le0.05$---so a signed-answer
build is accepted only with a volume-orthogonalised fold or an invariance penalty. We state this as an open
acceptance condition on the channel.
...
before it is final. \REDO{frozen-fold vs.\ tokens-in-tower concept curves on the fixed fold} The remaining open item is
the volume-leak waiver above, which fixes the final certified operating point.
...
B\i y\i k soft-attributes~\cite{biyik2023soft} & Gaussian belief $+$ EVOI & own elicitation task & the central threat; frozen-vs-co-trained wedge. \\
