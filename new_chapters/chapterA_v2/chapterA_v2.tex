...
on the validation cohort and scored on the disjoint test cohort. The combined bank \emph{contains} the items-only
bank, so at the optimum it cannot do worse; the combined-versus-items gap is therefore a readout of what the
concept channel adds to an interview rather than an artifact of whichever heuristics were paired. All three arms
...
\emph{Items win full accuracy on the long interview, and the combined bank does not repair it.} From
$q{=}8$ items lead on full ($.1863$ vs $.1776$), and at $q{=}16$ combined trails items-only by $-0.0049$
despite the combined bank \emph{containing} the item bank. That is not overfitting---the build-set and
held-out curves differ by $-0.0047$, $-0.0002$ and $-0.0025$ across the three arms, i.e.\ the test cohort
scores \emph{higher} than the cohort the sequences were built on. It is myopia: a greedy that maximises
each prefix commits its first four slots to concepts and cannot undo them by the sixteenth question, and
NDCG over a sequence is not guaranteed submodular. We therefore read the combined$-$items gap only where
the optimiser is not implicated---at $q\le4$, where the combined bank freely chose concepts over every
available item, and on the tail throughout.
