...
\begin{axis}[
  ...
  \node[star,star points=5,draw=green!45!black,fill=yellow!85!red,minimum size=11pt,inner sep=0pt]
        at (axis cs:4.5,0.373) {};
  \node[green!45!black,font=\scriptsize,align=center,anchor=north] at (axis cs:4.5,0.370)
  ...
  \node[font=\scriptsize,anchor=west] at (axis cs:2.60,0.3585) {\textbf{RecVAE}};
  ...
  \addplot[only marks,mark=o,mark size=2.6pt,color=gray!70!black] coordinates {
    (1.5,0.076) (2.0,0.076) (2.5,0.076) (3.0,0.076)
  };
  \node[gray!60!black,font=\tiny,anchor=south] at (axis cs:1.5,0.079) {SASRec/BERT4Rec};
  \node[gray!60!black,font=\tiny,anchor=north] at (axis cs:2.0,0.0745) {GATE, LLM-CRS, graph filters};
  \node[gray!60!black,font=\tiny,anchor=south] at (axis cs:2.5,0.079) {PEBOL, EDDI, TaNP};
  \node[gray!60!black,font=\tiny,anchor=north] at (axis cs:3.0,0.0745) {BCIE};
  \node[gray!60!black,font=\tiny,anchor=west] at (axis cs:3.55,0.086)
        {no full-profile number exists};
  \node[gray!60!black,font=\tiny,anchor=west] at (axis cs:3.55,0.079)
        {(off the $y$ scale; $x$ position only)};
\end{axis}
\end{tikzpicture}
...
Golbandi-node is the tree's best-effort node recommender). Hollow markers sit in the separated grey strip \emph{below}
the measured $y$-scale: these are systems for which \emph{no full-profile number exists}---unrun, unrunnable at full
catalogue, or not portable (see the master-table dispositions); only their $x$ position is meaningful. The shaded top-right box is the instrument target
