...
RecVAE~\cite{ren2020recvae}                  & \cmark & \cmark & \xmark & \pmark & \xmark & \multicolumn{2}{c}{---} & not-run$^{\dagger}$ & @100 $0.442$ (ours \textbf{PASS} $+.0005$) \\
...
RecVAE-class $d{=}512$ ruler                 & \pmark & \cmark & \xmark & \pmark & \xmark & $.4998$ & $.3443$ & recorded$^{\dagger}$ & n/a (in-house; ML-20M snap \textbf{PASS} $0.442$) \\
...
``n/r'' where the literature reports none. RecVAE$^{\dagger}$: not run standalone on ML-25M---its ML-25M
realisation is our $d512$ ruler row. No number is invented: a value appears only if it is in a baseline JSON, in
