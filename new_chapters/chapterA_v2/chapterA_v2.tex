...
\begin{description}[leftmargin=2.4em,itemsep=1.5pt]
  ...
  \item[G1c --- belief scale calibrated.] The belief magnitude correlates with held-out NLL. This is a
        \emph{diagnostic}, not an acceptance gate: it scores the posterior's scale, on which no result in this
        chapter depends, and no comparator in the literature reports it.
...
passes G1a, G1b, G4 and G9, misses G1c, and is partial on G8 (Table~\ref{tab:batteryresults}). Two
...
G1c & $\rho(\text{belief-}\sigma,\text{NLL})$ (diagnostic) & $\rho=0.2393$ & \emph{reported} \\
...
\bottomrule
\end{tabular}
\caption{\textbf{The acceptance battery, measured.} G1c is a diagnostic rather than an acceptance
gate (\S\ref{sec:reqs}) and is reported for completeness: the posterior is strongly directional
(G1b, $11.5\times$ against a $2\times$ bar) while its scale tracks held-out NLL only weakly.
