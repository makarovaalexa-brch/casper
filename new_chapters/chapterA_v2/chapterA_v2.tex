...
RecVAE~\cite{ren2020recvae} & \TODO{training} & $0.442$ & training \\
\bottomrule
\end{tabular}
...
EASE/RecVAE on the shared harness. The item-only baselines are measured (Table~\ref{tab:g0}); the EDLAE cell is
still running.
...
EDLAE~\cite{steck2020edlae} & \TODO{running} & \TODO{running} & running \\
\midrule
...
reuses the canonical harness (split/cohort/tail/seed protocol) verbatim, so EASE is on the same ruler as our
recorded anchors. EDLAE pending.}
\label{tab:g0}
\end{table}

\paragraph{EASE sets the G0 bar, and we report it as such.} On our ML-25M harness EASE attains $0.5078$ full /
$0.3394$ tail---\emph{above} our interview-native encoder ($0.4946/0.3372$). The evaluate path in the baseline
...
difference in the evaluate path. The instrument's R1 obligation is therefore to close a $\sim1.3$-point
full-profile gap to EASE, not merely to tie RecVAE. We report this rather than excluding the stronger baseline.
