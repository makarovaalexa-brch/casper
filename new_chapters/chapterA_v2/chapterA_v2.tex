...
remaining per-build gates have all been run and pass.
\end{abstract}
...
intercept $0.1279/0.0192$). They tell a consistent story: the signed answer repairs the regression, but the concept channel's value is \emph{shaped}---it owns the
...
$q{=}16$ ($.0776$ vs $.0679$, $+14\%$). This is worth more than it looks: \textbf{the greedy maximised full
NDCG@10 only}---the tail was never an objective in any arm---so the tail advantage is an incidental
property of the channel rather than something the optimiser was steered toward.
