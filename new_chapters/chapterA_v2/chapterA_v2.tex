...
RecVAE~\cite{ren2020recvae} (ours, paper config) & \cmark & \cmark & \xmark & \pmark & \xmark & $\mathbf{.3540}$ & $\mathbf{.2497}$ & measured & @100 $0.442$ (ours \textbf{PASS} $+.0005$) \\
...
ordinal capability count, the number of R1--R5 marks a system satisfies in the master table (\cmark${=}1$,
\pmark${=}0.5$)---and \emph{measured full-profile NDCG@10 on our ML-25M ruler} (R1) on the $y$-axis. The measured
points trace a \emph{descending} frontier: accuracy peaks at the item-only accuracy corner ($x{=}2.5$) and falls as
interview-compatibility rises, so the top-right---high compatibility \emph{and} a full-profile number---is empty.
...
\begin{axis}[
    ...
    xmin=0, xmax=5, ymin=0.10, ymax=0.38,
    xtick={0,1,2,3,4,5},
  ...
  % --- target region: top-right, x>=4 and y>=frontier bar (RecVAE 0.354) ---
  \fill[green!14] (axis cs:4,0.354) rectangle (axis cs:5,0.38);
  \draw[green!45!black,dashed] (axis cs:4,0.354) rectangle (axis cs:5,0.38);
  \node[green!45!black,font=\scriptsize,align=center] at (axis cs:4.5,0.368)
        {instrument target\\(T2$'$, \textcolor{red}{\textbf{TODO}})};
  ...
  \addplot[only marks,mark=*,mark size=2.6pt,color=blue!55!black] coordinates {
    ...
    (1.0,0.3065) (2.5,0.3433) (2.5,0.3476) (2.5,0.3540)
  };
  % measured-point labels
  ...
  \node[font=\scriptsize,anchor=west] at (axis cs:2.60,0.3400) {EDLAE};
  \node[font=\scriptsize,anchor=west] at (axis cs:2.60,0.3490) {EASE};
  \node[font=\scriptsize,anchor=west] at (axis cs:2.60,0.3585) {\textbf{RecVAE (ours)}};
  \node[font=\scriptsize,anchor=south west] at (axis cs:3.02,0.2019) {belief-MF (B\i y\i k/ConTS core)};
  % --- unmeasured / unrunnable systems: hollow, pinned at the axis floor ---
  \addplot[only marks,mark=o,mark size=2.6pt,color=gray] coordinates {
    (1.5,0.10) (2.0,0.10) (2.5,0.10) (3.0,0.10)
  };
  \node[gray,font=\tiny,rotate=90,anchor=west] at (axis cs:1.53,0.104) {SASRec/BERT4Rec};
  \node[gray,font=\tiny,rotate=90,anchor=west] at (axis cs:2.03,0.104) {GATE, LLM-CRS, graph filters};
  \node[gray,font=\tiny,rotate=90,anchor=west] at (axis cs:2.53,0.104) {PEBOL, EDDI, TaNP};
  \node[gray,font=\tiny,rotate=90,anchor=west] at (axis cs:3.03,0.104) {BCIE};
  \node[gray,font=\tiny,anchor=west] at (axis cs:3.25,0.108) {$\circ$ = no full-profile number exists};
  \node[gray,font=\tiny,anchor=west] at (axis cs:3.25,0.100) {(floor position, not a measurement)};
\end{axis}
\end{tikzpicture}
\caption{\textbf{The gap map (measured axes).} $y$-axis: measured full-profile NDCG@10 on our canonical ML-25M ruler.
$x$-axis: an \emph{interview-compatibility score}, defined as the number of R1--R5 marks a system carries in the
master table (Table~\ref{tab:master}), scoring \cmark${=}1$ and \pmark${=}0.5$. This score is a \emph{coarse ordinal
capability count, not a measurement}, so the $x$-axis is qualitative even though the $y$-axis is measured. Filled
markers are systems with a full-profile number on this ruler (belief-MF is the shared B\i y\i k/ConTS posterior core;
Golbandi-node is the tree's best-effort node recommender). Hollow markers, pinned at the axis floor, are systems for
which \emph{no full-profile number exists}---unrun, unrunnable at full catalogue, or not portable (see the master-table
dispositions); their height is not meaningful, only their $x$. The shaded top-right box is the instrument target
($x\ge4$ and full-profile at the frontier bar), still \TODO{}. The shape of the evidence is a \emph{collapse, not a
gradual trade-off}: the upper envelope is roughly flat up to the item-only accuracy corner ($x{=}2.5$:
RecVAE/EASE/EDLAE at $0.343$--$0.354$), then falls off a cliff --- the only measured system beyond it (belief-MF at
$x{=}3.0$) drops to $0.2019$, below plain matrix factorisation, and \emph{nothing measurable exists at $x\ge3.5$}. The
corner is not expensive; it is unoccupied. The
nearest published rival to the target is B\i y\i k et al.~\cite{biyik2023soft} (highest compatibility of any system
with an actionable belief), whose runnable core nonetheless measures at $0.2019$ full---far below the frontier bar.}
\label{fig:gapmap}
\end{figure}
...
RecVAE (ours, paper config)~\cite{ren2020recvae} & $\mathbf{0.3540}$ & $\mathbf{0.2497}$ & @100 $0.4537$; nominal frontier \\
\midrule
