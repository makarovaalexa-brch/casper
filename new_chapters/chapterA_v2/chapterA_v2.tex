...
\begin{axis}[
  ...
  \node[green!45!black,font=\scriptsize,align=center,anchor=north] at (axis cs:3.75,0.369)
        {\textbf{CASPER} (target)};
  % --- our certified tower: MEASURED (accuracy done, channels partial) ---
  ...
  \node[green!35!black,font=\scriptsize,align=center,anchor=south] at (axis cs:2.5,0.3535)
        {\textbf{T2$'$ tower} (ours, certified)};
  \draw[green!45!black,dashed,-Latex,thin,opacity=0.7] (axis cs:2.62,0.3492) -- (axis cs:3.62,0.3690);
...
with the dashed arrow marking the remaining distance to the target (the channel and belief marks it has yet to earn
in full). The population is otherwise \emph{bimodal}: the entire accuracy corner
...
\paragraph{Both channels carry an interview, and the concept channel is strongest exactly where the item channel is
weakest.} The instrument's claim on the elicitation side is deliberately modest: not that concepts beat items, but that
\emph{both} channels fold and both move the ranking, which is what makes the instrument usable for measuring policies
over a mixed question bank. Under a matched question budget---a realizable concept ask against a popularity-ranked item
ask, both scored on the same cold cohort against the $0.1279/0.0192$ intercept---the two channels separate cleanly by
regime (full/tail NDCG@10):
...
questions $k$ & 1 & 2 & 4 & 8 \\
\midrule
concept ask & $\mathbf{.1591}/\mathbf{.0638}$ & $\mathbf{.1803}/\mathbf{.0861}$ & $\mathbf{.1972}/\mathbf{.1050}$ & $.2107/\mathbf{.1210}$ \\
item ask    & $.1374/.0392$ & $.1493/.0478$ & $.1839/.0648$ & $\mathbf{.2274}/.0839$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The concept channel leads on \emph{full} accuracy through $k{=}4$ and dominates the \emph{tail} at every
budget measured---at $k{=}8$ the tail gap is $.1210$ versus $.0839$, about $+44\%$---while items overtake on full
accuracy only at $k{=}8$. The mechanism is the coarse-to-fine intuition made concrete: a concept question splits a cold
population along a taste axis in one move, where a single item barely moves the belief; and because a concept direction
is not anchored to any one popular title, its gains land disproportionately off the head, which is precisely the tail.
Two honest caveats attach. The comparison is a realizable concept ask against a \emph{popularity} item ask, which is
the item channel's strongest simple strategy but not its ceiling; and items do win full accuracy by $k{=}8$, so this is
a statement about \emph{shape}, not a claim that one channel dominates. What the instrument chapter needs from this is
only the weaker conclusion, which is unambiguous: an interview conducted entirely in out-of-catalogue concepts---a
channel no R1-bar baseline can express at all---moves a frontier-accuracy recommender substantially and monotonically
off its cold intercept.
...
answer plus a curriculum over the number of concepts and dropped members) raised capture to roughly $25\%$ and
inverted the long-interview reading, as the table above shows. We record the retraction rather than quietly restating
the numbers, because it is the same lesson the rest of this section is about: a channel's measured ceiling is a
property of the operator it is measured through, and a weak operator manufactures a confident negative result. The
full strategy$\times$channel picture---question-bank strategies, and the best \emph{static} question sequence obtained
by greedy NDCG maximisation over an items-only, concepts-only, and combined bank---is being re-measured on the fixed
fold with construction and evaluation on disjoint cohorts. \REDO{strategy$\times$channel suite and the best-static
greedy sequences, fixed fold, build on the 10k validation cohort / evaluate on the disjoint 10k test cohort; figure to
be inserted here}
