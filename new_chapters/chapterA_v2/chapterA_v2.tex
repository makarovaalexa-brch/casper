...
remaining per-build gates have all been run on a pre-certification snapshot of the same architecture---nine pass, one
misses, two are partial---and we report the miss and the two partials in the same detail as the passes. The
...
been run} on a pre-certification snapshot of the same architecture: nine pass, one misses, two are partial
(Table~\ref{tab:batteryresults}). Results measured on that earlier snapshot are labelled \REDO{} where the
...
\begin{axis}[
  ...
  \node[star,star points=5,draw=green!45!black,fill=yellow!85!red,minimum size=11pt,inner sep=0pt]
        at (axis cs:4.0,0.373) {};
  \node[green!45!black,font=\scriptsize,align=center,anchor=north east] at (axis cs:3.94,0.371)
  ...
  \draw[green!45!black,dashed,-Latex,thin,opacity=0.7] (axis cs:2.62,0.3492) -- (axis cs:3.88,0.3705);
  \node[green!45!black,font=\tiny,anchor=north,align=center] at (axis cs:3.25,0.3572)
...
row on the ML-25M ruler. This is the bridge doing its job: the same rule that licenses the EASE and RecVAE numbers
disqualifies the DAE one. We note for completeness that our iALS lands at $0.358$ against the published WMF anchor of
$0.386$, so the iALS row throughout should be read as a \emph{floor} for that model family rather than a tuned
reproduction of it.
...
G1a & $\mathrm{tr}\,\bSigma$ shrinks over answers & $1881.9\!\to\!1805.1$ across $q{=}0\ldots16$, monotone & \textbf{PASS} \\
G1b & directional, $\ge2\times$ & along-$\bphi$ $0.996$ vs elsewhere $0.086$ $=\mathbf{11.5\times}$ & \textbf{PASS} \\
G1c & $\rho(\text{belief-}\sigma,\text{NLL})\ge0.25$ & $\rho=0.2393$ & \emph{miss} \\
G2 & cold curve rises, beats random & $\bSigma$-arm $0.1279\!\to\!0.2724$ full, $0.0192\!\to\!0.1305$ tail; rise $+0.1445/+0.1113$ & \textbf{PASS} \\
G3a & sign-flip lowers NDCG & $-0.2985$ (CI $[-0.3047,-0.2922]$); intensity staircase $\rho=0.927$ & \textbf{PASS} \\
G3b & value-neutralisation costs & $-0.2869$ (CI $[0.2817,0.2922]$) full; $0.2999\!\to\!0.0368$ tail & \textbf{PASS} \\
G4 & refusal boundedness, same channel & $\alpha_{\text{refuse}}/\alpha_{\text{answer}}=0.0007$ vs a $\le0.05$ bar & \textbf{PASS} \\
...
\bottomrule
\end{tabular}
\caption{\textbf{The acceptance battery, measured.} $^\dagger$C1 passes \emph{with a disqualification}:
one of the four amortised anchors failed its snap and was dropped rather than carried
(\S\ref{sec:bridge}). G1c misses a bar that was itself set relative to an abandoned learned uncertainty
head ($\rho=0.219$); the analytic belief beats that head by $+0.020$ but falls $0.011$ short of the
rounded bar we set ourselves, so the honest statement is that the posterior is \emph{directionally}
excellent (G1b, $11.5\times$) and \emph{magnitude}-calibrated only weakly. G6's wrong-user arm does not
sit near the intercept as written; it falls far \emph{below} it---folding a stranger's answers is actively
harmful, which is stronger evidence of genuine ingestion than the gate's own criterion demanded, but it is
a failure of the criterion as stated and we record it as partial rather than reinterpret it into a pass.
G8 shows $\bSigma$ is load-bearing from $q{=}4$ onward but not at $q{=}2$. All rows but G0/C1 are on the
