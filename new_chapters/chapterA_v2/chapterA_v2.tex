...
interview-side results (the graded-input curve, the concept channel) are measured on the same architecture but on a
pre-certification snapshot and an earlier fold recipe; they are labelled as such throughout and their re-runs on the
certified tower are queued.
\end{abstract}
...
the certified tower} (\S\ref{sec:expdesign}, Table~\ref{tab:g0}); the remaining gate results in this chapter were
measured on a pre-certification snapshot of the same architecture and are labelled \REDO{} where their re-run on the
certified tower is queued, or \TODO{} where the gate has not yet been run at all.
...
whichever heuristics were paired. \REDO{best-static greedy sequences (items-only / concepts-only / combined), fixed
fold, built on the 10k validation cohort and evaluated on the disjoint 10k test cohort; this table and figure are the
chapter's concept-versus-item evidence and no cross-channel claim is made until they land}
...
preserves the tie once it is placed over the frozen latent, which is \TODO{} until the belief layer is fitted
(\S\ref{sec:method}).
