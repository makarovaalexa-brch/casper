...
same selector is pointed at items). We make this axis deployable with a \emph{second pillar}: instead of assuming who can
answer what, we \emph{measure} it with an external LLM judge that reads a user's known profile, validated by a
pre-registered four-test gate (heterogeneous, taste-tracking beyond popularity, and carrying user-specific signal a
matched popularity$+$genre rule does not---a $+44$-point validity gap), cross-checked against a second model family and an
independent collaborative-filtering predictor, and distilled into a calibrated $P(\text{answerable}\mid\text{features})$
model (held-out-user AUC $0.92$, ECE $0.008$). We then study a \emph{learned} reconstruction-trained policy, CASPER-R,
over the answerable action type. Its recipe is instructive---from a heuristic-distilled floor, a self-supervised
\emph{reconstruction} objective is the only refinement that sharpens the elicited belief, while REINFORCE and supervised
ranking both \emph{degrade} it---but its endpoint margin over the static heuristic does \emph{not} survive training-seed
variation: \textbf{we claim no learned endpoint win}, and report CASPER-R as a mechanism analysis (it matches the
heuristic on average). Finally, blind LLM askers show answerability is a \emph{non-obvious} lesson: model capability
predicts answerability-awareness (the strongest avoids items unprompted, the weakest does not). The carried
contributions are the \emph{answerability axis} and its \emph{external measurement}; CASPER-R is a single policy over a
unified item$+$concept embedding that is simultaneously the recommender.
\end{abstract}
...
(mostly unanswerable) item questions by $+50\%$ tail NDCG ($0.140$ vs $0.093$; \S\ref{sec:answerresults}). \emph{(iii)}
Because every result above depends on \emph{who can answer what}, we replace the assumed answer model with a
\emph{measured} one: an external LLM judge, validated by a pre-registered gate and cross-checks and distilled into a
calibrated $P(\text{answerable})$ model (\S\ref{sec:external}). A \emph{learned}, answerability-aware policy
(\textbf{CASPER-R}) over the unified item$+$concept pool---trained not by reward but by self-supervised
\emph{reconstruction}---sharpens the elicited belief and, given every lever, \emph{chooses} to ask zero items,
rediscovering the answerability argument as emergent behaviour (\S\ref{sec:learned}--\ref{sec:adapt}); but its endpoint
margin over the strong heuristic does \emph{not} survive training-seed variation, so we claim no learned endpoint win and
present it as a mechanism analysis. The paper's carried contributions are therefore the \emph{answerability axis} and its
\emph{external measurement}, not a learned-policy result.
...
\begin{enumerate}
  \item \emph{(Method, this paper's headline)} \textbf{Externally-sourced answerability}: rather than assume who can
  answer what, we \emph{measure} answerability with an external LLM judge reading each user's known profile, validated by
  a pre-registered four-test gate (heterogeneity, taste-tracking beyond popularity, a $+44$-point held-out validity gap
  showing user-specific signal beyond a matched popularity$+$genre rule, and a privileged exploitability upper bound),
  cross-checked against a second model family (Cohen's $\kappa{=}0.67$) and an independent CF value predictor, and
  distilled into a calibrated $P(\text{answerable}\mid\text{features})$ model (held-out-user AUC $0.92$, ECE $0.008$;
  \S\ref{sec:external}). This removes the self-authored circularity of assuming an answer model, and gives every
  downstream elicitation result a measured, two-model-robust answerability environment.
  \item \emph{(Method, mechanism analysis)} \textbf{CASPER-R}: an answerability-aware elicitation policy over the
  ...
  reward. Given items and concepts on equal footing it \emph{chooses to ask zero items}, rediscovering the answerability
  argument as emergent behaviour, and its reconstruction recipe sharpens the elicited belief where reward and ranking
  objectives degrade it. \textbf{We claim no endpoint win}: its tail margin over the strong concept heuristic does not
  survive training-seed variation (\S\ref{sec:learned}), so CASPER-R is reported as a mechanism analysis that
  \emph{matches} the heuristic, not beats it. We credit the unified item$+$attribute action space to ConTS (Li et al.\
  2021, \cite{conts}), which chose a \emph{discrete categorical} attribute set by Thompson sampling; our additions are the
  open-vocabulary \emph{concept} granularity and the \emph{reconstruction} training objective.
...
constraint---which we call the \emph{structural lower-bound answer model}, one of the two answerability models
used in this chapter---is a deliberately conservative, designer-authored floor: it counts only demonstrably
logged films and under-counts what a real user could answer. \S\ref{sec:external} replaces this assumption with
an externally \emph{measured} answer model (an LLM judge) and reports the load-bearing claims under both. This
...
cohort within each split).\footnote{These five seeds vary only the held-out \emph{user} split, not the policy
optimisation. When the policy is instead retrained under independent \emph{training} seeds, CASPER-R's tail
margin over the fair-entropy heuristic averages to $\approx0$ at $q8$ (\S\ref{sec:learned}); we therefore
claim no learned endpoint win and treat the CASPER-R analysis as descriptive.}
...
\section{Externally-sourced answerability: measuring the answer model instead of assuming it}\label{sec:external}
Every result so far, and every prior elicitation evaluation we know of, rests on a silent and un-deployable
answer model. Ours (\S\ref{sec:background}) declares a question \emph{answerable iff rated} (items) or iff the
user has $\ge2$ member items (concepts); simulator-based conversational systems answer from the recommender's
own latent geometry; both are assumptions the evaluator \emph{authored}, and a sharp reviewer can always ask
whether an adaptive method merely rediscovers the rule its designers wrote. The ``answerable iff rated'' rule
is best understood as a designer-authored \emph{lower bound} on knowledge---it counts only films the user
demonstrably logged, and grossly under-counts what a real user could actually answer (they have seen far more
than they rated). This section replaces the \emph{assumption} with a \emph{measurement}: an external LLM judge
that reads a user's known profile and judges what they could answer, authored by no rule of ours, and thus not
exploitable circularly. It removes \emph{self-authored} circularity; it does not remove model-prior dependence
(the LLM is a learned proxy over the same public opinion-space), so the claim boundary is stated explicitly at
the end and a human validation is left as future work.

\paragraph{Design (pre-registered, frozen before any run).} ML-25M; a pinned LLM judge (\texttt{gpt-5.4-mini},
temperature $0$); one fixed answerer split (seed $123$), cached and reused across all recommender-eval seeds;
``maybe''$\to$refuse (conservative). The judge sees only the \emph{known half} of the profile (never the
held-out targets), and held-out targets are excluded from the interview bank so no interview can leak a target's
rating. Answers stay non-circular: real ratings for rated items and pairs, a data-side member-rating aggregate
for concepts, and an LLM-predicted value only where no ground truth exists (sensitivity-only, never in a headline).
Four gate tests were pre-registered with cutoffs frozen before the run (Table~\ref{tab:extgate}).

\begin{table}[h]\centering\small
\begin{tabular}{lll l}
\toprule
gate test & rules out & cutoff & measured\\
\midrule
heterogeneity & ``everyone answers the same'' & ICC $\ge0.05$ & ICC $0.174$ (perm $p{=}.003$) \quad PASS\\
taste-tracking & ``answerability $=$ popularity'' & OR $\ge1.5$/sd or $\Delta$AUC $\ge.05$ & OR $2.71$/sd, $\Delta$AUC $.106$ \quad PASS\\
validity gap & ``LLM just stereotypes the prompt'' & $\ge15$pt low/mid tier & \textbf{$+44.1$pt} moderate ($n{=}768$) \quad PASS\\
G2 exploitability$^\ddagger$ & ``heterogeneity is useless for NDCG'' & CI$>0$, $\Delta$NDCG$\ge.005$ & $\Delta$NDCG $0.287$ CI$[.268,.307]$ \quad PASS\\
\bottomrule
\end{tabular}
\caption{The pre-registered four-test answerability gate (ML-25M, $300$ users, \$1.29). All four pass. The
\emph{validity gap} is load-bearing: for mid-popularity films the user did \emph{not} show the judge, the judge
rates them more answerable when the user actually rated them than when matched on popularity$+$genre---i.e.\ it
infers real user-specific knowledge from taste, not stereotype. $^\ddagger$G2 is a \emph{privileged upper bound}
(the selector sees the full answerability table and true taste), not the realizable agent prize: it sizes the
fuel, it does not claim it. The heterogeneity ICC quoted here ($0.174$) is a \emph{single-read, pre-equating}
estimate; \S\ref{sec:flutter} shows it is inflated by presentation flutter and reports the corrected flutter-free trait
ICCs ($0.043$ concept, $0.037$ attribute, $0.073$ item knows-well, $0.000$ item recognition). The gate passes under
both, but the corrected figures are the ones to cite.}
\label{tab:extgate}
\end{table}

\paragraph{Cross-checks (both pass).} Two robustness checks rule out single-source artifacts. \emph{Cross-family:}
re-judging with a different model family (\texttt{claude-haiku-4.5}) agrees on answerability $83.2\%$ of the time
(Cohen's $\kappa{=}0.67$, substantial) and replicates the validity-gap \emph{sign}, so the signal is a property of
capable LLMs reading profiles, not a one-model quirk. \emph{Independent CF value:} a bank-restricted EASE trained
on known-portion ratings predicts the same masked held-out ratings at MAE $0.74$ versus the LLM's $0.68$---two
independently-built predictors agreeing within $0.06$ stars, so the LLM value channel is not a shared-prior
artifact.

\paragraph{Main study: a fitted, calibrated $P(\text{answerable})$.} Because the agent needs answerability for
\emph{any} item or concept, not just a bank, we judge the full $200$-tag concept set plus a $1{,}716$-item
stratified sample ($108{,}224$ labeled observations) and fit a logistic $P(\text{answerable}\mid\text{features})$,
\emph{validated on held-out users} (fit on training users only): \textbf{AUC $0.921$} (grouped-5-fold $0.916$),
\textbf{ECE $0.008$} (well-calibrated across all ten bins). Standardized coefficients (Table~\ref{tab:extcoef})
put ratings-count and the concept flag first, but the load-bearing term is \emph{genre-match} ($+0.63$)---the
user-specific fuel that a popularity-only rule cannot see. Masked-item validation gives the counterfactual value
channel its fidelity noise: LLM MAE $0.697$ stars (independent-CF EASE $0.745$), i.e.\ a fidelity $\sigma\approx0.70$
stars, consistent across the gate, the cross-check, and the main study.

\begin{table}[h]\centering\small
\begin{tabular}{lccccc c}
\toprule
standardized coef.\ (logistic) & log ratings-count & is-concept & genre-match & decade & pop.\ pct.\ & franchise\\
\midrule
$\hat\beta$ & $+1.55$ & $+1.27$ & $\mathbf{+0.63}$ & $+0.37$ & $-0.27$ & $+0.02$\\
\bottomrule
\end{tabular}
\caption{Fitted $P(\text{answerable}\mid\text{features})$, standardized coefficients (base answer-rate $0.732$;
held-out-user AUC $0.92$, ECE $0.008$). Genre-match ($+0.63$) is the user-specific signal a popularity-only rule
omits; pop-percentile is negative because it is collinear with ratings-count. This fitted model is the
\emph{LLM-derived scale surrogate}---plumbing that scores answerability everywhere the interview asks---and is
\emph{never} cited as the independent structural witness; that role is reserved for a separate a-priori rule
(rated OR top-popularity for items; $\ge k$ rated members for concepts), fixed before looking at any LLM output.}
\label{tab:extcoef}
\end{table}

\subsection{Presentation flutter and the equated trait: how reliable is an LLM judge?}\label{sec:flutter}
The gate above (Table~\ref{tab:extgate}) reports a between-user heterogeneity ICC from a \emph{single} read of each
profile. Before trusting any per-subject rate we asked a prior question: is the judge's answer even repeatable when
the \emph{content} is held fixed and only the \emph{presentation} changes? \emph{(All quantities in this subsection
derive from the $173/300$-user answerer grid; a frozen-grid re-run is pending and these numbers are directional.)}

\paragraph{The shuffle probe.} We re-sent identical interview content to the judge three times, shuffling only
question order and the order of the profile lines, with no caching ($3$ users spanning the knowledge range, $250$
item-questions each, $9$ calls, \$0.05). Two things held and one moved. The \emph{value} channel is stable: on cells
judged known in two re-sends the star ratings agree to MAE $0.12$--$0.21$, comparable to their agreement with the
original read, and the \emph{within-call} ordering of items is preserved ($\approx0.99$). But the
\emph{knowledge-commitment threshold}---the ``knows it well'' versus ``rough idea'' cutoff---slides wholesale per
call: the know-well \emph{rate} on identical content spreads by up to $0.66$ across re-sends (the three users spread
by $0.66$, $0.54$, $0.16$). The judge does not change \emph{what} it thinks the viewer knows relatively; it changes
\emph{how much} of the scale it labels ``well'' from one call to the next.

\paragraph{A score-plus-wobbly-cutoff model.} This is the signature of a stable latent value read through a
\emph{fluctuating decision threshold}: each item carries a content-driven propensity, but the cutoff the judge applies
to turn that propensity into a discrete knowledge label drifts by a per-call offset. Absolute per-subject \emph{rates}
therefore inherit that offset; \emph{within-call contrasts} (near-vs-far, item-vs-item) do not.

\paragraph{Equating the threshold out.} We remove the per-call offset by \emph{test-equating}~\cite{kolen04,lord80},
the standard device for placing scores from different test forms onto a common scale. A joint per-call-intercept refit
is unidentifiable here---the production battery was never shuffled, so a cell's call is a deterministic function of
its question position and the call intercept is collinear with question-block difficulty---so we use a two-step
estimator. First we remove each \emph{question}'s mean answer rate, absorbing all content difficulty; then, on the
question-residualised outcome, a nested variance-components model~\cite{searle92} splits the remainder into a
flutter-free per-viewer \emph{trait} (constant across a viewer's calls), a between-call \emph{presentation flutter},
and a cell residual. Because content is removed first, any residual between-call, within-viewer variation is
presentation, not content (Table~\ref{tab:flutterdecomp}).

\begin{table}[h]\centering\small
\begin{tabular}{lrrrrr}
\toprule
channel (headline margin) & question & user-feat.\ & user-trait & call-flutter & cell resid.\\
\midrule
concept (knows-of-it)          & $54.6\%$ & $0.6\%$ & $1.3\%$ & $4.3\%$  & $39.3\%$\\
entity/attribute (knows-of-it) & $32.0\%$ & $0.0\%$ & $2.5\%$ & $16.2\%$ & $49.3\%$\\
item (knows-\emph{well})       & $34.3\%$ & $0.3\%$ & $4.5\%$ & $21.7\%$ & $39.2\%$\\
\bottomrule
\end{tabular}
\caption{Variance decomposition of the judge's knowledge answer (share of $\mathrm{Var}(y)$; question-residualised
nested variance components, $173$ users; headline margin per channel, item on the knows-\emph{well} cut since
knows-of-it is saturated at ${\sim}99\%$). \textbf{Call-flutter}---the presentation artefact the shuffle probe
exposed---is the largest user-side component on the attribute and item channels ($16$--$22\%$), dwarfing the stable
\emph{user-trait} ($1$--$5\%$). It is neither viewer signal nor content. Rows sum to $\approx100\%$ up to
crossed-design non-additivity.}
\label{tab:flutterdecomp}
\end{table}

\paragraph{The corrected trait.} A single-read one-way ICC attributes \emph{all} user-level grouping to trait; because
each viewer answers a channel over only a few calls, per-call flutter does not average out and inflates the apparent
between-user fuel. Using only the flutter-free component sharply revises the picture (Table~\ref{tab:fluttericc}). The
largest correction is the attribute channel, whose apparent heterogeneity ($0.092$) is mostly flutter and falls to
$0.037$. Most tellingly, item \emph{recognition} (knows-of-it) has \emph{zero} trait ICC once equated: recognising a
popular title is a property of the title, not a durable person dial. The item \emph{knows-well} margin retains a real
but roughly-halved trait ($0.137\to0.073$).

\begin{table}[h]\centering\small
\begin{tabular}{llrrr}
\toprule
channel & margin & old one-way ICC & corrected trait ICC & flutter var.\\
\midrule
concept & knows-of-it        & $0.034$ & $0.043$          & $0.008$\\
entity  & knows-of-it        & $0.092$ & $\mathbf{0.037}$ & $0.024$\\
item    & knows-\emph{well}  & $0.137$ & $\mathbf{0.073}$ & $0.046$\\
item    & recognition        & $0.016$ & $\mathbf{0.000}$ & $0.001$\\
\bottomrule
\end{tabular}
\caption{Corrected (flutter-free) trait ICC versus the old single-read one-way ICC, per channel. The one-way estimate
is inflated by presentation flutter; the corrected trait uses only the between-viewer component that survives equating.
Item recognition is \emph{not} a person trait ($0.000$)---a viewer-level dial exists only for the item
knows-\emph{well} and (weakly) the concept margins.}
\label{tab:fluttericc}
\end{table}

\paragraph{Two independent routes agree on the flutter's size.} The equating estimates the item knows-well call-flutter
variance at $0.046$ from question-residualised production data ($173$ users); the shuffle probe measures the same
quantity directly---between-call know-well-rate variance on identical content---at $0.068$ ($3$ users). These two
routes, one from re-sending identical content and one from population-scale residualisation, agree to the same order of
magnitude, cross-validating the flutter's magnitude (the probe estimate is higher and far noisier on $3$ users, two of
them extreme; the $173$-user equating is the number we cite).

\paragraph{Scope and implication.} This finding currently rests on a $3$-user shuffle probe plus a single variance
decomposition on the $173$-user grid. A formal repeat-study ($30$--$50$ users $\times\ge3$ shuffled re-sends, across
judge snapshots) to pin the channel-by-channel flutter profile and its temperature/snapshot dependence is
pre-registered future work, and no human-transfer or policy-value claim in this chapter leans on the corrected ICCs
beyond sizing the fuel. Framed honestly, this is a methods contribution with a stated evidence scope. The general
implication we draw is transferable: \emph{any} LLM-judge study reporting absolute per-subject rates from a single
read is exposed to this threshold flutter, whereas within-presentation contrasts are robust to it---a reason to prefer
contrast designs, or to equate, when an LLM is the measuring instrument.

\subsection{LLM world-knowledge distillation: what does the judge read in a rating profile?}\label{sec:distill}
The judge's per-user behaviour is, by construction, a function of the profile it was shown---titles, years, and
star values alone. Distilling that behaviour into an \emph{interpretable} feature model therefore doubles as a
study of which profile properties carry a person's answerability in the eyes of a knowledgeable reader: the
\emph{anatomy of judged answerability}. We fit regularized ordinal models over a full census of
profile-derivable features---volume; obscurity depth (percentile profile, niche shares \emph{and} counts);
era structure (spread, pre-1970 mass, per-question era distance to the user's rated-era mass); interest
breadth as a user property (taste-cloud dispersion, top-pocket concentration, effective pocket count);
distinctiveness (taste typicality, neighbourhood density); territory coverage of the question universe;
composition markers (foreign, franchise, documentary, niche-tag engagement); and rating style
(generosity, decisiveness)---every feature restricted to what the judge could actually see, selected by
cross-validated elastic net, with stability reported across leave-one-user-out folds. Direct reading of
extreme profiles motivates the design: the judge does not assign users a global ``knowledgeability'';
it \emph{matches territory}---a viewer deeply literate in 1930s--60s Hollywood is judged unable to discuss a
modern catalogue and vice versa---so per-question territory features, not user constants, carry the signal.
The census confirms the territory reading. Ranked by summed leave-one-user-out importance across channels, the
load-bearing profile features are \emph{era spread}, \emph{median obscurity} (rating-percentile depth),
\emph{pre-1970 depth}, \emph{catalogue coverage}, and \emph{franchise share} (Table~\ref{tab:distillfeat})---all
descriptors of \emph{where in the catalogue a viewer lives}, not a scalar ``knowledgeability''; signs are $\ge98\%$
consistent across the $173$ leave-one-user-out refits. But the honest out-of-sample verdict is blunt, and we state
it as such: a user-level census predicts \emph{concept} answerability across held-out users ($5$-fold CV
$R^2\approx32\%$), yet does \emph{not} cross-validate for the item or attribute channels---item CV $R^2\approx3\%$
(item knowledge is a question property, fame, not a person dial) and attribute CV $R^2$ is \emph{negative}
($\approx-28\%$: the census overfits it in-sample, with no reliable observable-profile predictor of per-user attribute
answerability beyond the question's own fame). The stable, generalising signal is the per-\emph{question}
content-difficulty axis (fame and co-knowledge for items, membership size and pop-weight for concepts, filmography
fraction for attributes, era alignment throughout), not a user constant.

\begin{table}[h]\centering\small
\begin{tabular}{lrrr}
\toprule
feature (LOUO fold-mean $\hat\beta$) & concept & entity & item\\
\midrule
era spread              & $-0.051$ & $-0.172$ & $-0.197$\\
median obscurity pct.\  & $+0.086$ & $-0.143$ & $-0.115$\\
pre-1970 depth (log-count) & $+0.062$ & $+0.103$ & $+0.164$\\
catalogue coverage      & $-0.052$ & $-0.040$ & $-0.200$\\
franchise share         & $-0.046$ & $+0.009$ & $-0.211$\\
\bottomrule
\end{tabular}
\caption{Top profile features by summed leave-one-user-out importance ($173$ refits; standardized fold-mean
coefficients, signs $\ge98\%$ consistent across folds). These are \emph{territory} descriptors. The user-level census
generalises out-of-sample for the \emph{concept} channel only ($5$-fold CV $R^2\ 32\%$); it does not cross-validate
for item ($3\%$) or attribute ($-28\%$), so per-user answerability there is not a stable, profile-readable trait.}
\label{tab:distillfeat}
\end{table}

\paragraph{Claim boundary (stated everywhere this environment is used).} The LLM judge removes the
\emph{self-authored} circularity the field's assumed answer models carry; it does \emph{not} establish
human-level answerability. Headline claims hold under \emph{two} external answer models---the LLM judge and the
a-priori structural rule---and under two model families and two value predictors; the fitted surrogate is plumbing,
never the independent witness. We never write ``realistic users'': the human study that validates the judge against
real answers is future work. The ``answerable iff rated'' model of \S\ref{sec:background} is thus one of two
answerability models used in this chapter---the conservative structural lower bound---and the external judge is the
measurement that extends and stress-tests it.
...
(\S\ref{sec:background}) and trained by \emph{reconstruction} (hence the ``R'')---and analyse what it does differently
from the heuristic. \textbf{We claim no learned endpoint win.} On a single, favourable training seed CASPER-R edges the
static heuristic on the tail ($+0.012$), but this margin \emph{does not survive training-seed variation}: retraining the
policy under three independent training seeds (the earlier ``seeds'' varied only the held-out \emph{user} split, not the
optimisation) leaves its mean tail margin over the fair-entropy heuristic at $q8$ statistically indistinguishable from
zero ($-0.001$, averaged over training seeds; the favourable seed gives $+0.008$ at $q2$/$+0.001$ at $q8$, a second seed
$-0.007$/$-0.003$). \textbf{Averaged over training seeds, CASPER-R matches the heuristic.} We therefore report this
section as a \emph{descriptive mechanism analysis}---what a reconstruction-trained policy does relative to a static
one---rather than as a performance win. The mechanism is real and reproducible even where the endpoint margin is not: the
policy asks the same question \emph{type} (both ask answerable concepts) but it \emph{understands} the user (its belief
reconstructs the taste vector more sharply, $\cos(u,u^\star)\approx0.74$ vs the heuristic's $0.64$), it \emph{front-loads}
(reaching the heuristic's eight-question score in $\sim\!2$ questions), and it \emph{personalises} (branching the question
sequence on the running answers instead of replaying a fixed shortlist). The robust contribution of this paper is the
\emph{answerability} axis (\S\ref{sec:answerresults}) and the externally-sourced answer model (\S\ref{sec:external});
the learned policy \emph{matches}, and does not beat, the static heuristic at the budget's end.
...
for model selection; head/tail by the Cremonesi split). A concept is answerable iff the user has $\ge 2$ items in it (the
structural lower-bound answer model of \S\ref{sec:background}; the external judge of \S\ref{sec:external} is the measured
alternative).
...
\bottomrule
\end{tabular}
\caption{\textbf{Main comparison} (single favourable training seed; seed-averaged over the $5$ held-out \emph{user}
splits, te$[300{:}]$, $q8$). \textbf{The CASPER-R tail margin shown here does not survive training-seed variation}
(\S\ref{sec:learned}): on this seed CASPER-R edges the tail (NDCG $\mathbf{0.152}$; next best the \emph{static} Golbandi
interview $0.129$ and the divisiveness heuristic $0.140$), but averaged over training seeds it matches the heuristic
($\approx0$ tail margin at $q8$); the full metric is a tie regardless ($\approx0.36$; entropy edges it $0.361$). The
answerability pattern below is training-free and robust. Three patterns. \emph{(i) Answerability}: concept askers ($0.11$--$0.14$ tail) dominate item askers; the
...
\paragraph{Reading.} On the single favourable training seed reported in Table~\ref{tab:learned}, CASPER-R edges the tail
on every belief-quality axis---tail NDCG $+0.012$, tail Recall $+0.012$, tail MRR $+0.019$, and a much sharper belief
($\cos\,0.745$ vs $0.636$, $+0.109$). \textbf{These margins do not survive training-seed variation} (the retraining
overturn above): averaged over independent training seeds the tail margin is $\approx0$, so we read the belief-cosine and
front-loading numbers as a \emph{characterisation of what reconstruction training does}, not as an endpoint win. Full
NDCG is in any case a clean \emph{tie} ($-0.001$; popularity-saturated for every strong method, $\approx0.36$). The one
axis that \emph{is} robust and training-free is answerability---concept askers dominate item askers regardless of
seed---which is the paper's carried claim; CASPER-R matches, and does not beat, the static heuristic at $q8$.
...
\bottomrule
\end{tabular}
\caption{Recipe ablation (NDCG@10, single favourable training seed, averaged over held-out user splits). Among refinement
objectives, reconstruction is the only one that lifts the entropy floor on this seed; RL and ranking-SL \emph{degrade}
it, and dropping the floor (from-scratch) collapses. Read as a \emph{relative} comparison of objectives: the absolute
reconstruction lift over the floor does not survive training-seed variation (\S\ref{sec:learned}), but the ordering
(reconstruction $>$ RL $>$ ranking; pretraining essential) is the reproducible finding.}
\label{tab:ablation}
\end{table}
...
this \emph{per-user re-ordering}---not a richer vocabulary or a different question \emph{type}---which is the mechanism by
which reconstruction training sharpens the belief ($\cos\,0.745$ vs $0.636$) while asking the same \emph{kind} of
question (this belief-sharpening is reproducible; the endpoint tail margin it buys is not, \S\ref{sec:learned}).
...
\bibitem{kolen04} Kolen, Brennan. Test Equating, Scaling, and Linking: Methods and Practices. 2nd ed., Springer, 2004.
\bibitem{lord80} Lord. Applications of Item Response Theory to Practical Testing Problems. Lawrence Erlbaum, 1980.
\bibitem{searle92} Searle, Casella, McCulloch. Variance Components. Wiley, 1992.
\end{thebibliography}
\end{document}
