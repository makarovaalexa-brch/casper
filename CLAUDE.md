...
- **Canonical full-profile ruler (2026-07-21, author-approved): the Liang-recipe strong-generalization split applied to ML-25M** — ratings >3.5, min_uc=5, **10,000 val + 10,000 test held-out users** (train = remainder), per-user 80/20 fold-in/target, seed 98765; built by `scripts/baselines/liang_split.py --data ml-25m`. Primary metric: full AND tail NDCG@10 (tail = top-33%-popularity mask from train counts); NDCG@100 secondary. The old 500/500 arena split is for elicitation-line continuity ONLY — never for G0/full-profile claims. Study-user quarantine: dropped (author ruling, Jul 21).
