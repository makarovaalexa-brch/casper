...
| **Old Paper B ruler-vs-method (archaeology + design)** | was the old pre-RecVAE short-interview lift (+0.08/+0.127 full/tail; concepts +36% tail) a stronger METHOD or a RULER artifact | **faithful fresh run BLOCKED** (old biased-SVD/encoder/concept weights all gone — `.cache/` empty & never git-tracked; **ML-1M raw absent**). Old answers were GEOMETRIC not LLM. **Strong prior: RULER artifact** (weak recommender × dense small ML-1M × huge cold→oracle headroom 0.28→0.51 × item cold-unanswerability 1.9/8); new strong stack absorbs it (concepts lose to items, coarseness ceiling cos≈0.83). 2×2 dataset×recommender rebuild pre-registered, awaits author sign-off | `design/DESIGN_PAPERB_REPLICATION.md`, mem `paperB-old-ruler-vs-method-verdict` |
