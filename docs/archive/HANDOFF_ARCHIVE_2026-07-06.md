...
## 13. TWO MORE USER CRITIQUES (2026-07-06) — repeat-probing realism + the item-snap answerability hole
Both extend §8/§9/§10; both are load-bearing for how R2/P4c are worded, and each has a cheap decisive test.

### 13.1 REPEAT-PROBING IS OVER-FLATTERED BY THE NOISE MODEL ("who answers the same question 5× differently?")
The R2 winner (noise-adapted static, schedule `[0,0,0,0,1,2,7,0]`, 0.3143) denoises by RE-ASKING the top
axis — but that only works because the simulator draws a FRESH i.i.d. noise sample per question
(`a = 1.55s − 0.36 + N(0,0.48²)`, resampled each ask). A real human re-asked the IDENTICAL question returns
the SAME answer → averaging buys nothing. Real channel variance = (i) re-randomizing noise (mood, phrasing,
misclicks) — repetition helps; (ii) FIXED model-mismatch (their true taste on that axis is what it is) —
repetition helps ZERO. Our model treats ALL variance as (i), maximally flattering repeat-probing.
- CONSEQUENCE: the R2 SUB-claim "repeat-static BEATS the actor" (0.314 vs 0.284) is FRAGILE — likely shrinks
  as the fixed-noise fraction rises. The HEADLINE "discrete answerable > continuous under noise" is SAFE
  (concept-8/item-8 ask DISTINCT questions, no repetition).
- LEGITIMATE version to keep: repeat the AXIS via DIFFERENT surface phrasings (semi-independent reads), not
  the identical question. Say this in the paper.
- DECISIVE EXPERIMENT (E1): decompose channel noise into fixed-per-(user,axis) σ_f + resampled σ_r; sweep
  the mix; find where repeat-probing stops winning. Report the crossover. Cheap. Human study (§9.2 test-retest
  on repeated seed items) measures the real σ_f/σ_r split → anchors the sweep.

### 13.2 THE ITEM-SNAP "+0.051 UNDER NOISE" IS MEASURED WITHOUT AN ANSWERABILITY GATE (inconsistent w/ Part 3)
P4c Part 1 reports item-snap HELPS under noise (unsnapped 0.150 → item-snap 0.201, +0.051) while concept-snap
HURTS (−0.088). But Part 3 shows freely-chosen snapped items are answerable only 0.3% of the time. Reconcile:
Part 1 item-snap computes the GEOMETRIC/channel answer for whatever item q lands on, with NO check the user
has seen it = implicit UNIVERSAL-answerability assumption; the +0.051 lives entirely in the 99.7% Part 3
calls unanswerable. NOT a per-user leak — the OPPOSITE: it assumes all items answerable. So "item-snap
denoises" is NOT realizable as stated; it's query-regularization measured under a free answer oracle.
- ACTION (verify first, ~2 min): confirm in `scripts/instrument2/p4c_answer_sources.py` that the Part-1
  item-snap arm applies NO answerability filter. If confirmed, label the +0.051 row "no answerability gate".
- DECISIVE EXPERIMENT (E2): re-run snap-denoising snapping to the nearest ANSWERABLE target under the T5b
  popularity-conditioned answerability model — (a) nearest POPULAR/known item, (b) nearest BROAD concept/
  phrase (richer bank than the coarse one that gave −0.088). Does snapping still denoise when the snap target
  must be answerable? If yes → deployable finding; if it collapses like Part 3 → retract "item-snap helps".
- CHANNEL SHOWCASE: item triangulation (Part 3) was only the ZERO-MODEL robustness floor (raw ratings need
  items). The DEPLOYABLE showcase channel is CONCEPTS/broad phrases (answerable by anyone) — foreground that
  in C/D; keep item-pairs as the conservative no-model floor. (Consistent w/ §8a, §10.3.)

### 13.3 Decision for the user
E1 + E2 (+ the 2-min code verify) are both cheap and directly harden C/D against exactly these reviewer
shots. Recommend running both before the T2/T8 write so the corrected wording is evidence-backed, not just
hedged. Not yet launched — awaiting go-ahead (squeeze R3 rungs #16/#18/#20 still finishing).
